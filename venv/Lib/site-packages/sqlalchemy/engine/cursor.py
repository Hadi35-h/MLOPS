...
class CursorResultMetaData(ResultMetaData):
    ...
    def _for_freeze(self) -> ResultMetaData:
        ambiguous = {
            rec[MD_LOOKUP_KEY] for rec in self._keymap.values() if rec[MD_INDEX] is None
        }
    ...
    def __init__(
        ...
        if context.result_column_struct:
            ...
            result_columns = cols_are_ordered = num_ctx_cols = (  # type: ignore
                ad_hoc_textual
        ...
        if num_ctx_cols:
            ...
            if len(by_key) != num_ctx_cols:
                ...
                # record into by_key.
                by_key.update(
                    {key: (None, -1, (), key, key, None, None) for key in dupes}
                )
...
class _NoResultMetaData(ResultMetaData):
    ...
    def _we_dont_return_rows(self, err: Optional[BaseException] = None) -> NoReturn:
...
class CursorResult(Result[_T]):
    ...
    def last_updated_params(
        self,
    ) -> Union[List[_MutableCoreSingleExecuteParams], _MutableCoreSingleExecuteParams]:
