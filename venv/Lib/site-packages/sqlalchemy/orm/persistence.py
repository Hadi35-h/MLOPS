    ...
    for state, state_dict, mapper, connection in states_to_insert:
        ...
        if not bulk or return_defaults:
            ...
            if mapper.base_mapper._prefer_eager_defaults(connection.dialect, table):
                has_all_defaults = mapper._server_default_col_keys[table].issubset(
                    params
                )
            else:
        ...
        if hasvalue:
            ...
            if not allow_executemany:
                ...
                check_rowcount = enable_check_rowcount and (
                    assert_multirow or (assert_singlerow and len(multiparams) == 1)
                )
    ...
    for (
        ...
        if (
            (
                ...
                or (
                    has_all_defaults
                    or not base_mapper._prefer_eager_defaults(connection.dialect, table)
