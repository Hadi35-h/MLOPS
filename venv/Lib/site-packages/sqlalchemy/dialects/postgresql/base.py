...
class PGCompiler(compiler.SQLCompiler):
    ...
    def visit_sequence(self, seq, **kw):
        return "nextval(%s)" % self.preparer._format_sequence_string_literal(seq)
        ...
        for c in cols:
            ...
            if coercions._is_literal(value):
                ...
                if isinstance(value, elements.BindParameter) and value.type._isnull:
                    value = value._clone()
                    value.type = c.type
            value_text = self.process(value.self_group(), is_upsert_set=True, **set_kw)
...
class PGIdentifierPreparer(compiler.IdentifierPreparer):
    ...
    def _format_sequence_string_literal(self, sequence):
        ...
        """
        return "'%s'" % self.format_sequence(sequence, use_schema=True).replace(
            "'", "''"
        )
    ...
    def _index_query(self):
        ...
        idx_sq = (
            select(
                ...
                sql.func.unnest(pg_catalog.pg_index.c.indkey).label("attnum"),
                sql.func.unnest(pg_catalog.pg_index.c.indclass).label("att_opclass"),
                sql.func.generate_subscripts(pg_catalog.pg_index.c.indkey, 1).label(
                    "ord"
                ),
            )
