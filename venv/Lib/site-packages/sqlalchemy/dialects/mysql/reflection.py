...
class MySQLTableDefinitionParser:
    ...
    def __init__(self, dialect: MySQLDialect, preparer: MySQLIdentifierPreparer):
    ...
    def _parse_constraints(self, line: str) -> Union[
        ...
        Tuple[Literal["ck_constraint", "fk_constraint", "key"], Dict[str, str]],
    ]:
    ...
    def _parse_partition_options(self, line: str, state: ReflectedState) -> None:
        options = {}
        new_line = line[:]
    ...
    def _parse_keyexprs(self, identifiers: str) -> List[Tuple[str, Optional[int], str]]:
        """Unpack '"col"(2),"col" ASC'-ish strings into components."""

        return [
            (colname, int(length) if length else None, modifiers)
            for colname, length, modifiers in self._re_keyexprs.findall(identifiers)
        ]
...
@overload
def _pr_compile(regex: str, cleanup: None = None) -> Tuple[re.Pattern[Any], None]: ...
...
_control_char_regexp = re.compile("|".join(re.escape(k) for k in _control_char_map))
