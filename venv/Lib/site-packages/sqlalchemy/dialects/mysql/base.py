...
class MySQLCompiler(compiler.SQLCompiler):
    ...
    def visit_aggregate_strings_func(self, fn: aggregate_strings, **kw: Any) -> str:
        expr, delimiter = (elem._compiler_dispatch(self, **kw) for elem in fn.clauses)
    ...
    def _render_json_extract_from_binary(
        ...
        if binary.type._type_affinity is sqltypes.Integer:
            ...
            if binary_type.scale is not None and binary_type.precision is not None:
    ...
    def render_literal_value(self, value: Optional[str], type_: TypeEngine[Any]) -> str:
    ...
    def get_select_precolumns(self, select: selectable.Select[Any], **kw: Any) -> str:
    ...
    def for_update_clause(self, select: selectable.GenerativeSelect, **kw: Any) -> str:
    ...
    def limit_clause(self, select: selectable.GenerativeSelect, **kw: Any) -> str:
        # MySQL supports:
        #   LIMIT <limit>
        #   LIMIT <offset>, <limit>
...
class MySQLDDLCompiler(compiler.DDLCompiler):
    ...
    def get_column_specification(self, column: sa_schema.Column[Any], **kw: Any) -> str:
        """Builds column DDL."""
        if (
    ...
    def visit_create_index(self, create: ddl.CreateIndex, **kw: Any) -> str:  # type: ignore[override]  # noqa: E501
        ...
        if length is not None:
            if isinstance(length, dict):
                ...
                columns_str = ", ".join("%s(%d)" % (col, length) for col in columns)
        else:
    ...
    def visit_drop_constraint(self, drop: ddl.DropConstraint, **kw: Any) -> str:
    ...
    def visit_set_table_comment(self, create: ddl.SetTableComment, **kw: Any) -> str:
    ...
    def visit_drop_table_comment(self, drop: ddl.DropTableComment, **kw: Any) -> str:
        return "ALTER TABLE %s COMMENT ''" % (self.preparer.format_table(drop.element))

    def visit_set_column_comment(self, create: ddl.SetColumnComment, **kw: Any) -> str:
...
class MySQLDialect(default.DefaultDialect):
    ...
    def _parse_server_version(self, val: str) -> Tuple[int, ...]:
        ...
        self._set_mariadb(bool(server_version_info and is_mariadb), server_version_info)
    ...
    def do_recover_twophase(self, connection: Connection) -> List[Any]:
        ...
        return [row["data"][0 : row["gtrid_length"]] for row in resultset.mappings()]
    ...
    def _extract_error_code(self, exception: DBAPIModule.Error) -> Optional[int]:
        raise NotImplementedError()
    ...
    def initialize(self, connection: Connection) -> None:
        ...
        self._connection_charset: Optional[str] = self._detect_charset(connection)
        ...
        self.use_mysql_for_share = self._is_mysql and self.server_version_info >= (
            8,
            0,
            1,
        )

        self._needs_correct_for_88718_96365 = self.server_version_info >= (8,) and (
            self.server_version_info < (8, 0, 14) or self._casing == 2
        )
    ...
    def _correct_for_mysql_bugs_88718_96365(
        ...
        schema_by_table_by_column: DefaultDict[str, DefaultDict[str, List[str]]] = (
        ...
        **kw: Any,
        ...
        for spec in parsed_state.keys:
            ...
            if flavor == "UNIQUE":
                ...
                util.warn("Converting unknown KEY type %s to a plain KEY", flavor)
...
class _DecodingRow:
    ...
    def __init__(self, rowproxy: Row[Any], charset: Optional[str]):
        ...
        self.charset = (
            self._encoding_compat.get(charset, charset) if charset is not None else None
        )
