    ...
    def create_connect_args(self, url: URL) -> ConnectArgsType:
        ...
        if "odbc_connect" in keys:
            ...
            def check_quote(token: str) -> str:
                if ";" in str(token) or "}" in str(token) or str(token).startswith("{"):
            ...
            if dsn_connection:
                ...
                if driver is None and keys:
                    ...
                    connectors.append("DRIVER={%s}" % str(driver).replace("}", "}}"))
            ...
            connectors.extend(["%s=%s" % (check_quote(k), v) for k, v in keys.items()])
