...
class TablesTest(TestBase):
    ...
    def _teardown_each_tables(self):
        ...
        if (
            ...
            with self.bind.begin() as conn:
                provision.delete_from_all_tables(conn, config, self._tables_metadata)
        ...
        for a, b in itertools.combinations_with_replacement(range(len(case_a)), 2):
            ...
            if isinstance(case_a[a], ClauseElement) and isinstance(
                ...
                eq_(
                    ...
                    sorted(util.unique_list(assert_a_params), key=lambda b: b.key),
                )
                eq_(
                    sorted(b_key.bindparams, key=lambda b: b.key),
                    sorted(util.unique_list(assert_b_params), key=lambda b: b.key),
                )
