...
class TestGreenlet(TestCase):
    ...
    def test_switching_many_threads(self):
        ...
        def run_it(thread_count=16):
            ...
            creators = [
                threading.Thread(
                    target=creates_greenlet, args=(greenlets, greenlets_all_created)
                )
            ...
            switchers = [
                threading.Thread(
                    target=switches_from_other_thread,
                    args=(greenlets, greenlets_all_created, all_threads_dead),
                )
                for _ in range(thread_count)
            ]
...
class TestModule(TestCase):
    ...
    @unittest.skipUnless(
        hasattr(sys, "_is_gil_enabled"), "Needs 3.13 and above for sys._is_gil_enabled"
    )
