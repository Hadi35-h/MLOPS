...
class FileResponse(Response):
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        send_pathsend = scope_type == "http" and "http.response.pathsend" in scope.get(
            "extensions", {}
        )
    ...
    def _should_use_range(self, http_if_range: str) -> bool:
        return (
            http_if_range == self.headers["last-modified"]
            or http_if_range == self.headers["etag"]
        )
    ...
    def _parse_ranges(cls, range_: str, file_size: int) -> list[tuple[int, int]]:
        ...
        for part in range_.split(","):
            ...
            try:
                start = (
                    int(start_str) if start_str else max(file_size - int(end_str), 0)
                )
                end = (
                    int(end_str) + 1
