...
class RequestBodyLimitResponder:
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        previous_scope_limit = cast(
            int | _Missing, scope.get(MAX_BODY_SIZE_SCOPE_KEY, _MISSING)
        )
        scope[MAX_BODY_SIZE_SCOPE_KEY] = self.max_body_size

        active_responder = cast(
            RequestBodyLimitResponder | None, scope.get(_BODY_LIMIT_RESPONDER_SCOPE_KEY)
        )
