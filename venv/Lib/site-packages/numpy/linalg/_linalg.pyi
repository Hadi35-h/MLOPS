...
_NumericScalarT = TypeVar(
    "_NumericScalarT", bound=np.number | np.timedelta64 | np.object_
)
...
@overload
def outer(
    x1: _ArrayLike[_NumberT], x2: _ArrayLike[_NumberT], /
) -> NDArray[_NumberT]: ...
@overload
def outer(
...
@overload
def eigvalsh(
    a: _ArrayLikeInt_co, UPLO: L["L", "U", "l", "u"] = "L"
) -> NDArray[float64]: ...
@overload
def eigvalsh(
    a: _ArrayLikeComplex_co, UPLO: L["L", "U", "l", "u"] = "L"
) -> NDArray[floating]: ...
@overload
...
def svdvals(  # type: ignore[overload-overlap]
    x: (
        _ArrayLike[np.float64 | np.complex128 | np.integer | np.bool]
