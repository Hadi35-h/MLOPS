...
class TestDelete:
    ...
    def _check_inverse_of_slicing(self, indices):
        ...
        assert_array_equal(setxor1d(a_del, a[indices,]), a, err_msg=msg)
...
class TestTrimZeros:
    ...
    def construct_input_output(self, rng, shape, axis, trim):
        ...
        idx = tuple(
            slice(i, j) if ax in axis else slice(None)
...
class TestQuantile:
    ...
    def test_inf_zeroes_err(self, weights, dty):
        ...
        with pytest.raises(
            ValueError, match=r"Weights included NaN, inf or were all zero"
        ):
    ...
    @pytest.mark.parametrize("weights", [[1, np.nan, 1, 1], [1, np.nan, np.nan, 1]])
    @pytest.mark.parametrize(
        ["err", "dty"], [(ValueError, "f8"), ((RuntimeWarning, ValueError), "O")]
    )
    ...
    def test_quantile_gh_29003_Fraction(self):
        ...
        r = np.quantile([1, 2], q=Fraction(0.5))
