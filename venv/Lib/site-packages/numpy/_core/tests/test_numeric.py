...
class TestStdVar:
    ...
    def test_ddof1(self):
        A, real_var = self._create_data()
        assert_almost_equal(np.var(A, ddof=1), real_var * len(A) / (len(A) - 1))
        assert_almost_equal(np.std(A, ddof=1) ** 2, real_var * len(A) / (len(A) - 1))

    def test_ddof2(self):
        A, real_var = self._create_data()
        assert_almost_equal(np.var(A, ddof=2), real_var * len(A) / (len(A) - 2))
        assert_almost_equal(np.std(A, ddof=2) ** 2, real_var * len(A) / (len(A) - 2))

    def test_correction(self):
        A, _ = self._create_data()
        assert_almost_equal(np.var(A, correction=1), np.var(A, ddof=1))
        assert_almost_equal(np.std(A, correction=1), np.std(A, ddof=1))
...
class TestCreationFuncs:
    def check_function(self, func, fill_value=None):
        ...
        par = ((0, 1, 2), range(ndims), orders, dtypes)
        fill_kwarg = {}
