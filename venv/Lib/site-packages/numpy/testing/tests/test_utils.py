...
class TestArrayEqual(_GenericTest):
    ...
    def test_masked_scalar(self):
        ...
        for a_val, b_val in itertools.product(
            [3.0, np.nan, -np.inf],
            itertools.product([3.0, 4.0, np.nan, np.inf, -np.inf], repeat=2),
        ):
            a = np.ma.MaskedArray(a_val, mask=True)
            b = np.array(b_val)
            self._test_equal(a, b)
...
class TestArrayAlmostEqual(_GenericTest):
    ...
    def test_complex_inf(self):
        a = np.array([np.inf + 1.0j, 2.0 + 1.0j, 3.0 + 1.0j])
        b = a.copy()
        self._assert_func(a, b)
        b[1] = 3.0 + 1.0j
        expected_msg = (
...
class TestAssertAllclose:
    ...
    def test_infs(self):
        ...
        with pytest.raises(AssertionError, match=re.escape(expected_msg)):
            assert_allclose(a, b)
        b = np.array([complex(np.inf, 1.0)])
        expected_msg = "inf values mismatch:"
        with pytest.raises(AssertionError, match=re.escape(expected_msg)):
            assert_allclose(a, b)

        a = np.array([complex(np.inf, 1.0)])
        b = np.array([complex(np.inf, 1.0)])
        assert_allclose(a, b)

        b = np.array([complex(np.inf, 2.0)])
        expected_msg = "inf values mismatch:"
        with pytest.raises(AssertionError, match=re.escape(expected_msg)):
            assert_allclose(a, b)
