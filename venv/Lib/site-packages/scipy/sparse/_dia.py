...
class _dia_base(_data_matrix):
    ...
    def _getnnz(self, axis=None):
        ...
        return int(
            np.maximum(
                np.minimum(M + self.offsets, L) - np.maximum(self.offsets, 0), 0
            ).sum()
        )
    ...
    def tocsr(self, copy=False):
        ...
        nnz = dia_tocsr(
            ...
            n_cols,
            *self.data.shape,
            self.offsets.astype(idx_dtype, copy=False),
            self.data,
            order,
            csr_data,
            indices,
        ...
        out = self._csr_container(
            (csr_data, indices, indptr), shape=self.shape, dtype=self.dtype
        )
