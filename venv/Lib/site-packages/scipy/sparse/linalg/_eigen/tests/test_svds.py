...
def _check_svds_n(
    A, k, u, s, vh, which="LM", check_res=True, check_svd=True, atol=1e-10, rtol=1e-7
):
    n, m = A.shape
...
class SVDSCommonTests:
    ...
    def test_svds_diff0_docstring_example(self):
        ...
        se = 2.0 * np.sin(np.pi * np.arange(1, 4) / (2.0 * n))
        ue = np.sqrt(2 / n) * np.sin(
            np.pi * np.outer(np.arange(1, n), np.arange(1, 4)) / n
        )
    ...
    @pytest.mark.filterwarnings("ignore:Exited", reason="Ignore LOBPCG early exit.")
    ...
    def test_svds_parameter_tol(self):
        ...
        def err(tol):
            _, s2, _ = svds(
                A, k=k, v0=np.ones(n), maxiter=1000, solver=self.solver, tol=tol, rng=0
            )
    ...
    def test_svd_return_singular_vectors(self, rsv, shape):
        ...
        if self.solver == "lobpcg":
            with pytest.warns(UserWarning, match="The problem size"):
                if rsv is False:
                    s2 = svds(
                        A, k, return_singular_vectors=rsv, solver=self.solver, rng=rng
                    )
    ...
    @pytest.mark.filterwarnings(
        "ignore:k >= N - 1", reason="needed to demonstrate #16725"
    )
    ...
    @pytest.mark.parametrize("lo_type", (np.asarray, csc_array, aslinearoperator))
    ...
    def test_svd_linop(self):
        ...
        for n, m, k in nmks:
            ...
            if self.solver == "lobpcg":
                with pytest.warns(UserWarning, match="The problem size"):
                    U1, s1, VH1 = reorder(
                        svds(A, k, which="SM", solver=solver, rng=0, **kwargs)
                    )
                    U2, s2, VH2 = reorder(
                        svds(L, k, which="SM", solver=solver, rng=0, **kwargs)
                    )
            else:
                U1, s1, VH1 = reorder(
                    svds(A, k, which="SM", solver=solver, rng=0, **kwargs)
            ...
            if k < min(n, m) - 1:
                ...
                for dt, eps in [(complex, 1e-7), (np.complex64, 3e-3)]:
                    ...
                    if self.solver == "lobpcg":
                        with pytest.warns(UserWarning, match="The problem size"):
                            U1, s1, VH1 = reorder(
                                svds(A, k, which="LM", solver=solver, rng=0)
                            )
                            U2, s2, VH2 = reorder(
                                svds(L, k, which="LM", solver=solver, rng=0)
                            )
                    else:
                        U1, s1, VH1 = reorder(
                            svds(A, k, which="LM", solver=solver, rng=0)
