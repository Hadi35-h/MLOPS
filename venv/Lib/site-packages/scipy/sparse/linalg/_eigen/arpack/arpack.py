...
class _ArpackParams:
    ...
    def _raise_no_convergence(self):
        ...
        raise ArpackNoConvergence(
            f"No convergence ({num_iter} iterations, "
...
class _SymmetricArpackParams(_ArpackParams):
    def __init__(
        ...
        _ArpackParams.__init__(
            self, n, k, tp, self.rng, mode, sigma, ncv, v0, maxiter, which, tol
        )
    ...
    def iterate(self):
        self._arpack_solver(
            self.arpack_dict, self.resid, self.v, self.ipntr, self.workd, self.workl
        )
        ...
        if self.arpack_dict["ido"] == 1:
            ...
            self.resid[:] = self.rng.uniform(low=-1.0, high=1.0, size=[self.n]).astype(
                self.tp
            )
            ...
            if self.arpack_dict["info"] == 0:
                ...
                raise ArpackError(
                    self.arpack_dict["info"], infodict=self.iterate_infodict
                )
...
class _UnsymmetricArpackParams(_ArpackParams):
    def __init__(
        ...
        if which not in _NEUPD_WHICH:
            raise ValueError(
                "Parameter which must be one of" f" {' '.join(_NEUPD_WHICH)}"
            )
        if k >= n - 1:
        ...
        self.rng = np.random.default_rng(rng)
        _ArpackParams.__init__(
            self, n, k, tp, rng, mode, sigma, ncv, v0, maxiter, which, tol
        )
    ...
    def iterate(self):
        ...
        if self.arpack_dict["ido"] == 1:
            ...
            if self.tp in "fd":
                # Generate random vector into resid
                self.resid[:] = self.rng.uniform(
                    low=-1.0, high=1.0, size=[self.n]
                ).astype(self.tp)
            else:
                # Generate complex random vector into resid
                self.resid[:] = (
                    self.rng.uniform(low=-1.0, high=1.0, size=[self.n, 2])
                    .view(np.complex128)
                    .astype(self.tp)
                    .ravel()
                )
            ...
            if self.arpack_dict["info"] == 0:
                ...
                raise ArpackError(
                    info=self.arpack_dict["info"], infodict=self.iterate_infodict
                )
...
def eigs(
    ...
    if sigma is None:
        ...
        matvec = aslinearoperator(A).matvec
        if Minv is not None:
            raise ValueError("Minv should not be specified when sigma is" "specified.")
        if OPinv is None:
