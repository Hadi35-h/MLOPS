...
def _get_test_tolerance(type_char, mattype=None, D_type=None, which=None):
    ...
    if (
        isinstance(mattype, type)
        and issubclass(mattype, csr_array)
...
def generate_matrix(
    N, complex_=False, hermitian=False, pos_definite=False, sparse=False, rng=None
):
    M = rng.random((N, N))
    if complex_:
...
class SymmetricParams:
    def __init__(self):
        ...
        Ar = (
            generate_matrix(N, hermitian=True, pos_definite=True, rng=rng)
            .astype("f")
            .astype("d")
        )
        M = (
        ...
        Ac = (
            generate_matrix(
                N, hermitian=True, pos_definite=True, complex_=True, rng=rng
            )
            .astype("F")
            .astype("D")
        )
...
@pytest.mark.parametrize(
    "sigma, mode",
    [(None, "normal"), (0.5, "normal"), (0.5, "buckling"), (0.5, "cayley")],
)
...
@pytest.mark.parametrize(
    "sigma, OPpart", [(None, None), (0.1, "r"), (0.1 + 0.1j, "r"), (0.1 + 0.1j, "i")]
)
@pytest.mark.parametrize("mattype", [csr_array, aslinearoperator, np.asarray])
@pytest.mark.parametrize("which", ["LM", "LR", "LI"])
@pytest.mark.parametrize("typ", ["f", "d"])
@pytest.mark.parametrize("D", NonSymmetricParams().real_test_cases)
def test_real_nonsymmetric_modes(D, typ, which, mattype, sigma, OPpart):
...
def test_eigsh_for_k_greater():
    ...
    M_sparse = generate_matrix_symmetric(4, pos_definite=True, sparse=True, rng=rng)
    M_linop = aslinearoperator(M_dense)
...
def test_gh24358(dtype):
    ...
    A = csr_array(
        np.array([[-1.3, 2.7, 0.2], [0.8, 4.1, 2.2], [2.1, 4.4, -1.9]], dtype=dtype)
    )
