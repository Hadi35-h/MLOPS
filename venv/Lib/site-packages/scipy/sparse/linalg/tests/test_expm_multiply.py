...
REAL_DTYPES = (np.intc, np_long, np.longlong, np.float32, np.float64, np.longdouble)
...
class TestExpmActionSimple:
    ...
    def test_complex(self):
        ...
        observed = expm_multiply(A, B)
        expected = np.array(
            [1j * np.exp(1j) + 1j * (1j * np.cos(1) - np.sin(1)), 1j * np.exp(1j)],
            dtype=complex,
        )
        assert_allclose(observed, expected)
        with estimated_warns():
...
class TestExpmActionInterval:
    ...
    def test_sparse_expm_multiply_interval(self):
        ...
        for num in (14, 13, 2):
            ...
            for target in (B, v):
                X = expm_multiply(
                    A, target, start=start, stop=stop, num=num, endpoint=endpoint
                )
                samples = np.linspace(
    ...
    def test_expm_multiply_interval_vector(self):
        ...
        for num, n in product([14, 13, 2], [1, 2, 5, 20, 40]):
            ...
            Xgiven = expm_multiply(
                aslinearoperator(A), v, num=num, **interval, traceA=np.trace(A)
            )
            # test robustness for linear operator with wrong trace
            Xwrong = expm_multiply(
                aslinearoperator(A), v, num=num, **interval, traceA=np.trace(A) * 5
            )
...
def test_expm_multiply_dtype(dtype_a, dtype_b, b_is_matrix):
    ...
    assert_allclose_ = (
        partial(assert_allclose, rtol=1.8e-3, atol=1e-5)
        if {dtype_a, dtype_b} & IMPRECISE
        else assert_allclose
    )
    rng = np.random.default_rng(1234)
    # test data
    n = 7
    ...
    if dtype_a in REAL_DTYPES:
        ...
        A = scipy.linalg.inv(rng.random([n, n]) + 1j * rng.random([n, n])).astype(
            dtype_a
        )
