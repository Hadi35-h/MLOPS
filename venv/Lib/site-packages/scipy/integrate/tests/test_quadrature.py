...
class TestCumulative_trapezoid:
    def test_1d(self, xp):
        ...
        y_expected = xp.asarray([0.0, -1.5, -2.0, -1.5, 0.0])
    ...
    def test_x_none(self, xp):
        ...
        y_expected = xp.asarray([-1.5, -2.0, -1.5, 0.0])
        xp_assert_close(y_int, y_expected)

        y_int = cumulative_trapezoid(y, initial=0)
        y_expected = xp.asarray([0, -1.5, -2.0, -1.5, 0.0])
        xp_assert_close(y_int, y_expected)

        y_int = cumulative_trapezoid(y, dx=3)
        y_expected = xp.asarray([-4.5, -6.0, -4.5, 0.0])
        xp_assert_close(y_int, y_expected)

        y_int = cumulative_trapezoid(y, dx=3, initial=0)
        y_expected = xp.asarray([0, -4.5, -6.0, -4.5, 0.0])
        xp_assert_close(y_int, y_expected)
...
class TestCumulativeSimpson:
    ...
    def test_nd(self, axis, x_ndim, x_len, i_ndim, dx, xp):
        ...
        ref = cumulative_simpson_nd_reference(
            np.asarray(y),
            x=np.asarray(x),
            dx=None if dx is None else np.asarray(dx),
            initial=None if initial is None else np.asarray(initial),
            axis=axis,
        )
    ...
    def test_simpson_exceptions(self, message, kwarg_update, xp):
        kwargs0 = dict(
            y=xp.asarray(self.y0), x=xp.asarray(self.x0), dx=None, initial=None, axis=-1
        )
        kwarg_update = {
            k: xp.asarray(np.asarray(v)) if isinstance(v, list) else v
    ...
    def test_special_cases(self, xp):
        ...
        res = cumulative_simpson(y, dx=0.0)
    ...
    def test_cumulative_simpson_against_simpson_with_default_dx(self, y, xp):
        ...
        xp_assert_close(
            res[..., 1:],
            xp.asarray(ref[..., 1:] + theoretical_difference[..., 1:]),
            atol=1e-16,
        )
    ...
    def test_cumulative_simpson_against_simpson(self, y, xp):
        """Theoretically, the output of `cumulative_simpson` will be identical
        to `simpson` at all even indices and in the last index. The first index
        will not match as `simpson` uses the trapezoidal rule when there are only two
        data points. Odd indices after the first index are shown to match with
