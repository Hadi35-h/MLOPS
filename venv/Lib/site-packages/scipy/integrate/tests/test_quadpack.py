...
class TestMultivariateCtypesQuad:
    ...
    def test_indefinite(self):
        ...
        assert_quad(
            quad(self._multivariate_indefinite, 0, np.inf), 0.577215664901532860606512
        )
...
class TestQuad:
    ...
    def test_sine_weighted_infinite(self):
        ...
        ome = 3.0
        assert_quad(
            quad(myfunc, 0, np.inf, args=a, weight="sin", wvar=ome),
            ome / (a**2 + ome**2),
        )
    ...
    def test_cosine_weighted_infinite(self):
        ...
        ome = 2.3
        assert_quad(
            quad(myfunc, -np.inf, 0, args=a, weight="cos", wvar=ome),
            a / (a**2 + ome**2),
        )
    ...
    def test_b_equals_a(self, complex_func):
        ...
        zero, err, infodict = quad(
            f, lower, upper, full_output=1, complex_func=complex_func
        )
        assert (zero, err) == (0.0, 0.0)
        if complex_func:
            assert_equal(
                infodict, {"real": expected_infodict, "imag": expected_infodict}
            )
        else:
...
class TestTplquad:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            # Multiple integration of a function in n = 3 variables: f(x, y, z)
            # over domain D = [-inf, -1] for each n (one at a time).
            (-np.inf, -1, -np.inf, 0, -np.inf, 0, (np.pi ** (3 / 2)) / 8 * erfc(1)),
            (-np.inf, 0, -np.inf, -1, -np.inf, 0, (np.pi ** (3 / 2)) / 8 * erfc(1)),
            (-np.inf, 0, -np.inf, 0, -np.inf, -1, (np.pi ** (3 / 2)) / 8 * erfc(1)),
            # Multiple integration of a function in n = 3 variables: f(x, y, z)
            ...
            # Multiple integration of a function in n = 3 variables: f(x, y, z)
            # over domain D = [-inf, 1] for each n (one at a time).
            (-np.inf, 1, -np.inf, 0, -np.inf, 0, (np.pi ** (3 / 2)) / 8 * (erf(1) + 1)),
            (-np.inf, 0, -np.inf, 1, -np.inf, 0, (np.pi ** (3 / 2)) / 8 * (erf(1) + 1)),
            (-np.inf, 0, -np.inf, 0, -np.inf, 1, (np.pi ** (3 / 2)) / 8 * (erf(1) + 1)),
            # Multiple integration of a function in n = 3 variables: f(x, y, z)
            # over domain D = [-inf, 1] for each n (two at a time).
            (
                -np.inf,
            ...
            # Multiple integration of a function in n = 3 variables: f(x, y, z)
            # over domain D = [1, inf] for each n (one at a time).
            (1, np.inf, 0, np.inf, 0, np.inf, (np.pi ** (3 / 2)) / 8 * erfc(1)),
            (0, np.inf, 1, np.inf, 0, np.inf, (np.pi ** (3 / 2)) / 8 * erfc(1)),
            (0, np.inf, 0, np.inf, 1, np.inf, (np.pi ** (3 / 2)) / 8 * erfc(1)),
            # Multiple integration of a function in n = 3 variables: f(x, y, z)
            # over domain D = [1, inf] for each n (two at a time).
            (1, np.inf, 1, np.inf, 0, np.inf, (np.pi ** (3 / 2)) / 8 * (erfc(1) ** 2)),
            (1, np.inf, 0, np.inf, 1, np.inf, (np.pi ** (3 / 2)) / 8 * (erfc(1) ** 2)),
            (0, np.inf, 1, np.inf, 1, np.inf, (np.pi ** (3 / 2)) / 8 * (erfc(1) ** 2)),
            # Multiple integration of a function in n = 3 variables: f(x, y, z)
            # over domain D = [1, inf] for all n.
            (1, np.inf, 1, np.inf, 1, np.inf, (np.pi ** (3 / 2)) / 8 * (erfc(1) ** 3)),
            # Multiple integration of a function in n = 3 variables: f(x, y, z)
            # over domain D = [-1, inf] for each n (one at a time).
            (-1, np.inf, 0, np.inf, 0, np.inf, (np.pi ** (3 / 2)) / 8 * (erf(1) + 1)),
            (0, np.inf, -1, np.inf, 0, np.inf, (np.pi ** (3 / 2)) / 8 * (erf(1) + 1)),
            (0, np.inf, 0, np.inf, -1, np.inf, (np.pi ** (3 / 2)) / 8 * (erf(1) + 1)),
            # Multiple integration of a function in n = 3 variables: f(x, y, z)
            # over domain D = [-1, inf] for each n (two at a time).
            (
                -1,
            ...
            # Multiple integration of a function in n = 3 variables: f(x, y, z)
            # over domain D = [-inf, inf] for all n.
            (-np.inf, np.inf, -np.inf, np.inf, -np.inf, np.inf, np.pi ** (3 / 2)),
            # Multiple integration of a function in n = 3 variables: f(x, y, z)
            # over domain D = [0, 0] for each n (one at a time).
    ...
    def test_triple_integral_improper(
        self, x_lower, x_upper, y_lower, y_upper, z_lower, z_upper, expected
    ):
        # The Gaussian Integral.
        def f(x, y, z):
