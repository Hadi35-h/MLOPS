...
def cumulative_trapezoid(y, x=None, dx=1.0, axis=-1, initial=None):
    ...
    if initial is not None:
        ...
        shape[axis] = 1
        res = xp.concat(
            (xp.full(tuple(shape), initial, dtype=res.dtype), res), axis=axis
        )
...
def _cumulatively_sum_simpson_integrals(
    ...
    sub_integrals_h2 = xp.flip(
        integration_func(xp.flip(y, axis=-1), xp.flip(dx, axis=-1)), axis=-1
    )
...
def qmc_quad(func, a, b, *, n_estimates=8, n_points=1024, qrng=None, log=False):
    ...
    def std(estimates, m=None, ddof=0, log=False):
        ...
        if log:
            ...
            return xp.real(0.5 * (logsumexp(2 * diff) - math.log(n_estimates - ddof)))
        else:
    ...
    if xp.any(a == b):
        ...
        return QMCQuadResult(zero, xp.asarray(0.0, dtype=a.dtype))
