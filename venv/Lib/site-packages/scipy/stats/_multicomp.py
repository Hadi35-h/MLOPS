...
class DunnettResult:
    ...
    def __str__(self):
        ...
        for i in range(self.pvalue.size):
            s += (
                f" (Sample {i} - Control) {self.statistic[i]:>10.3f}"
    ...
    def _allowance(
        ...
        allowance = (
            critical_value
            * self._std
            * np.sqrt(1 / self._n_samples + 1 / self._n_control)
        )
...
def dunnett(
    *samples: "npt.ArrayLike",  # noqa: D417
    ...
    samples_, control_, rng = _iv_dunnett(
        samples=samples, control=control, alternative=alternative, rng=rng
    )
...
def _statistic_dunnett(
    ...
    s2 = (
        np.sum(
            [
                _var(sample, mean=mean) * sample.size
                for sample, mean in zip(all_samples, all_means)
            ]
        )
        / df
    )
