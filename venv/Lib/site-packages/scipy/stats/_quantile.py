...
def _quantile_iv(x, p, method, axis, nan_policy, keepdims, weights):
    ...
    n_zero_weight = (
        n_zero_weight if n_zero_weight is None else xp.moveaxis(n_zero_weight, axis, -1)
    )
...
def _quantile_winsor(y, p, n, method, xp):
    ops = dict(
        round_outward=(xp.floor, xp.ceil),
...
def _quantile_bc(y, p, n, method, xp):
    ...
    if method == "_midpoint":
        return (
            xp.take_along_axis(y, xp.astype(xp.floor(ij), xp.int64), axis=-1)
            + xp.take_along_axis(y, xp.astype(xp.ceil(ij), xp.int64), axis=-1)
        ) / 2
    elif method == "_lower":
        k = xp.floor(ij)
    elif method == "_higher":
