...
def sobol_indices(*, func, n, dists=None, method="saltelli_2010", rng=None):
    ...
    if callable(func):
        ...
        try:
            f_A, f_B, f_AB = map(
                lambda arr: arr.copy(),
                np.atleast_2d(func["f_A"], func["f_B"], func["f_AB"]),
            )
        except KeyError as exc:
            raise ValueError(message) from exc
        ...
        if (
            f_A.shape[1] != n
            or f_A.shape != f_B.shape
            or f_AB.shape == f_A.shape
            or f_AB.shape[-1] % n != 0
        ):
            raise ValueError(message)
