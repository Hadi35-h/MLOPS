...
def epps_singleton_2samp(x, y, t=(0.4, 0.8), *, axis=0):
    ...
    x = xp.where(i_x, 1.0, x)
    y = xp.where(i_y, 1.0, y)
...
def _psi1_mod(x, *, xp=None):
    ...
    def _ed3(y):
        ...
        kv_terms = xp.asarray(2 * kv(1 / 4, z_) + 3 * kv(3 / 4, z_) - kv(5 / 4, z_))
...
def _cdf_cvm(x, n=None, *, xp=None):
    ...
    if n is None:
        ...
        y = xpx.at(y)[sup].set(
            _cdf_cvm_inf(x[sup], xp=xp) * (1 + 1.0 / (12 * n))
            + _psi1_mod(x[sup], xp=xp) / n
        )
        y = xpx.at(y)[x >= n / 3].set(1.0)
...
@xp_capabilities(
    cpu_only=True,  # needs special function `kv`
...
def cramervonmises(rvs, cdf, args=(), *, axis=0):
    ...
    p = xp.clip(1.0 - _cdf_cvm(w, n), 0.0, None)
...
def _pval_cvm_2samp_asymptotic(t, N, nx, ny, k, *, xp):
    ...
    p = xpx.apply_where(
        tn >= 0.003,
        (tn,),
        lambda tn: xp.clip(1.0 - _cdf_cvm_inf(tn, xp=xp), 0.0),
        fill_value=1.0,
    )
    return p
...
def cramervonmises_2samp(x, y, method="auto", *, axis=0):
    ...
    u = nx * xp.sum(
        (rx - xp.arange(1, nx + 1, dtype=dtype)) ** 2, axis=-1
