...
def _wilcoxon_statistic(d, method, zero_method="wilcox", *, xp):
    ...
    if zero_method == "pratt":
        ...
        mn = xpx.at(mn)[...].subtract(n_zero * (n_zero + 1.0) * 0.25)
        se = xpx.at(se)[...].subtract(n_zero * (n_zero + 1.0) * (2.0 * n_zero + 1.0))
...
def _wilcoxon_nd(
    ...
    if method == "asymptotic":
        ...
        if alternative == "less":
            ...
            p = 2 * np.minimum(
                dist.sf(np.floor(r_plus_np)), dist.cdf(np.ceil(r_plus_np))
            )
            p = np.clip(p, 0, 1)
        ...
        p = stats.permutation_test(
            (d,),
            lambda d: _wilcoxon_statistic(d, method, zero_method, xp=xp)[0],
