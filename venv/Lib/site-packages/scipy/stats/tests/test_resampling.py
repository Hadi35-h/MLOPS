...
class TestBootstrap:
    ...
    def test_bootstrap_against_theory(self, method, xp):
        ...
        xp_assert_close(
            res.confidence_interval.high, xp.asarray(dist.ppf(alpha)), rtol=5e-4
        )

        config.update(dict(n_resamples=0, bootstrap_result=res))
        res = bootstrap(**config, confidence_level=alpha, alternative="greater")
        xp_assert_close(
            res.confidence_interval.low, xp.asarray(dist.ppf(1 - alpha)), rtol=5e-4
        )
    ...
    def test_multisample_BCa_against_R(self, xp):
        ...
        res_basic = stats.bootstrap(
            (x, y), statistic, method="basic", batch=100, rng=rng
        )
    ...
    def test_BCa_acceleration_against_reference(self, xp):
        ...
        y = xp.asarray([10.0, 27.0, 31.0, 40.0, 46.0, 50.0, 52.0, 104.0, 146.0])
        z = xp.asarray([16.0, 23.0, 38.0, 94.0, 99.0, 141.0, 197.0])
        ...
        _, _, a_hat = _resampling._bca_interval(
            data, statistic, axis, alpha, theta_hat_b, batch, xp
        )
    ...
    def test_bootstrap_against_itself_1samp(self, method, expected, xp):
        ...
        pvalue = stats.binomtest(
            int(ci_contains_true), n_replications, confidence_level
        ).pvalue
        assert pvalue > 0.1
    ...
    def test_bootstrap_against_itself_2samp(self, method, expected, xp):
        ...
        res = bootstrap(
            (xp.asarray(data1), xp.asarray(data2)),
            statistic=my_stat,
            confidence_level=confidence_level,
            n_resamples=n_resamples,
            batch=50,
    ...
    def test_bootstrap_vectorized_3samp(self, method, axis, dtype, xp):
        ...
        res1 = bootstrap(
            (xp.asarray(x), xp.asarray(y), xp.asarray(z)),
            statistic,
            vectorized=True,
            axis=axis,
            n_resamples=100,
    ...
    def test_bootstrap_vectorized_1samp(self, method, axis, xp):
        ...
        xp_assert_close(
            res1.confidence_interval.low, xp.asarray(res2.confidence_interval.low)
        )
        xp_assert_close(
            res1.confidence_interval.high, xp.asarray(res2.confidence_interval.high)
        )
        xp_assert_close(res1.standard_error, xp.asarray(res2.standard_error))

    @pytest.mark.parametrize("method", ["basic", "percentile", "BCa"])
    def test_bootstrap_degenerate(self, method, xp):
        data = xp.full((35,), 10000.0)
        if method == "BCa":
            ...
            xp_assert_equal(res.confidence_interval.low, xp.asarray(10000.0))
            xp_assert_equal(res.confidence_interval.high, xp.asarray(10000.0))
        xp_assert_equal(res.standard_error, xp.asarray(0.0))
    ...
    def test_bootstrap_gh15678(self, method, xp):
        ...
        xp_assert_close(
            res.confidence_interval.low, xp.asarray(ref.confidence_interval.low)
        )
        xp_assert_close(
            res.confidence_interval.high, xp.asarray(ref.confidence_interval.high)
        )
    ...
    def test_bootstrap_alternative(self, method, xp):
        ...
        xp_assert_close(
            l.confidence_interval.high, t.confidence_interval.high, rtol=1e-14
        )
        xp_assert_close(
            g.confidence_interval.low, t.confidence_interval.low, rtol=1e-14
        )
    ...
    @pytest.mark.skip_xp_backends(
        "array_api_strict", reason="Test uses ... + fancy indexing"
    )
    ...
    def test_percentile_of_score(self, score, axis, xp):
        ...
        p = _resampling._percentile_of_score(
            xp.asarray(x, dtype=dtype), xp.asarray(score, dtype=dtype), axis=-1, xp=xp
        )
    ...
    def test_vector_valued_statistic(self, method, xp):
        ...
        def statistic(data, axis):
            return xp.stack(
                [xp.mean(data, axis=axis), xp.std(data, axis=axis, correction=1)]
            )
...
class TestPower:
    ...
    def test_input_validation(self, xp):
        ...
        with pytest.raises((ValueError, RuntimeError), match=message):
            power(
                test,
                rvs,
                (xp.asarray([10, 11]), xp.asarray([12, 13])),
    ...
    def test_batch(self, xp):
        ...
        def test(x, axis):
            ...
            test.counter += 1
            return stats.ttest_1samp(x, xp.asarray(0.0), axis=axis).pvalue

        test.counter = 0
        test.batch_size = 0

        kwds = dict(test=test, n_observations=xp.asarray(10), n_resamples=1000)
    ...
    def test_vectorization(self, xp):
        ...
        def test(x, alternative, axis=-1):
            ...
            popmeans_expanded = xpx.expand_dims(
                popmeans, axis=tuple(range(1, x.ndim + 1))
            )
    ...
    @pytest.mark.skip_xp_backends(
        cpu_only=True, exceptions=["cupy", "jax.numpy", "dask.array"]
    )
    ...
    @pytest.mark.skip_xp_backends(
        "array_api_strict", reason="currently combines integer and float arrays"
    )
    @pytest.mark.skip_xp_backends(
        cpu_only=True, exceptions=["cupy", "jax.numpy", "dask.array"]
    )
...
class TestPermutationTest:
    ...
    def test_permutation_test_iv(self, xp):
        ...
        with pytest.raises(ValueError, match=message):
            permutation_test(data, stat, permutation_type="ekki")
    ...
    def test_against_ks_2samp(self, alternative, xp):
        ...
        res = permutation_test(
            (x, y), statistic, n_resamples=np.inf, alternative="greater", rng=self.rng
        )
    ...
    def test_against_ansari(self, alternative, xp):
        ...
        x, y = xp.asarray(x), xp.asarray(y)
        res = permutation_test(
            (x, y), statistic, n_resamples=np.inf, alternative=alternative, rng=self.rng
        )
    ...
    def test_vectorized_nsamp_ptype_both(self, axis, xp):
        ...
        # Check results
        xp_assert_close(
            res.statistic, xp.asarray(expected.statistic), rtol=self.rtol * 5
        )
    ...
    def test_against_wilcoxon(self, alternative, xp):
        ...
        if alternative != "two-sided":
            xp_assert_close(
                res2.statistic, xp.asarray(expected.statistic), rtol=self.rtol
            )
    ...
    def test_with_ties(self, case, xp):
        ...
        with warnings.catch_warnings():
            ...
            res = permutation_test(
                (x, y), statistic, n_resamples=np.inf, alternative="less"
            )
            res2 = permutation_test(
                (x, y), statistic, n_resamples=np.inf, alternative="two-sided"
            )
        ...
        Pr_gte_S_mean = (
            xp.astype(
                xp.count_nonzero(
                    xp.abs(res.null_distribution - mean) >= xp.abs(S - mean)
                ),
                S.dtype,
            )
            / n
    ...
    def test_against_spearmanr_in_R(self, alternative, expected_pvalue, xp):
        ...
        y = xp.asarray(
            [2.71414076, 0.2488, 0.87551913, 2.6514917, 2.01160156, 0.47699563]
        )
    ...
    def test_finite_precision_statistic(self, xp):
        ...
        # for this in some way.
        x = xp.asarray([1.0, 2.0, 4.0, 3.0], dtype=xp.float64)
        y = xp.asarray([2.0, 4.0, 6.0, 8.0], dtype=xp.float64)

        def statistic(x, y, axis):
            return stats.pearsonr(x, y, axis=axis)[0]

        res = stats.permutation_test((x, y), statistic, permutation_type="pairings")
