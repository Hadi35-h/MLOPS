...
class TestQuantile:
    ...
    @skip_xp_backends(
        cpu_only=True, reason="PyTorch doesn't have `betainc`.", exceptions=["cupy"]
    )
    ...
    @skip_xp_backends(
        cpu_only=True,
        reason="PyTorch doesn't have `betainc`.",
    ...
    @pytest.mark.parametrize("weights", [False, True])
    def test_against_reference_2(
        self, axis, keepdims, nan_policy, dtype, method, weights, xp
    ):
    ...
    def test_zero_weights(self, method, xp):
        ...
        weights = xp.where(i_zero, 0.0, weights)
        ...
        weights_zeroed = xp.where(i_zero, 0.0, weights)
        res = stats.quantile(x, p, weights=weights_zeroed, method=method, axis=-1)
        ref = stats.quantile(
            x_nanned, p, weights=weights, nan_policy="omit", method=method, axis=-1
        )
        xp_assert_close(res, ref)
        xp_assert_equal(x, x0)  # no input mutation
