...
class TestChatterjeeXi:
    @pytest.mark.parametrize(
        ...
        [
            ...
            dict(
                y_cont=False, statistic=0.07407407407407396, pvalue=0.3709859367123997
            ),
        ],
    )
    ...
    def test_against_R_XICOR(self, case, dtype, xp):
        ...
        y = rng.random(size=10) if case["y_cont"] else rng.integers(0, 5, size=10)
    ...
    def test_permutation_asymptotic(self, y_continuous):
        ...
        y = rng.random(size=shape) if y_continuous else rng.integers(0, 10, size=shape)
        method = stats.PermutationMethod(rng=rng)
        res = stats.chatterjeexi(
            x, y, method=method, y_continuous=y_continuous, axis=-1
        )
