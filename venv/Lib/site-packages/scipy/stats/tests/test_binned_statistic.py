...
class TestBinnedStatistic:
    ...
    def test_dd_range_errors(self):
        ...
        with assert_raises(
            ValueError, match="In dimension 1 of range, start must be <= stop"
        ):
            binned_statistic_dd([self.x, self.y], self.v, range=[[1, 0], [0, 1]])
        with assert_raises(
            ValueError, match="In dimension 2 of range, start must be <= stop"
        ):
            binned_statistic_dd([self.x, self.y], self.v, range=[[0, 1], [1, 0]])
        with assert_raises(
            ValueError, match="range given for 1 dimensions; 2 required"
        ):
    ...
    def test_gh14332(self):
        ...
        sum1, edges1, bc = binned_statistic_dd(
            x, np.ones(len(x)), bins=[bins], statistic="sum"
        )
