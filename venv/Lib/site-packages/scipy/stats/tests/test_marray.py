...
@pytest.mark.parametrize(
    "fun",
    [
        make_xp_pytest_param(stats.zscore),
...
@pytest.mark.parametrize(
    "f",
    [
        make_xp_pytest_param(stats.ttest_1samp),
...
@pytest.mark.parametrize(
    "f",
    [
        make_xp_pytest_param(stats.skewtest),
...
@pytest.mark.skipif(
    version.parse(np.__version__) < version.parse("2"),
...
def test_directional_stats(xp):
    ...
    xp_assert_close(
        res.mean_resultant_length.data, xp.asarray(ref.mean_resultant_length)
    )
...
@pytest.mark.parametrize(
    "f",
    [make_xp_pytest_param(stats.pearsonr), make_xp_pytest_param(stats.pointbiserialr)],
)
