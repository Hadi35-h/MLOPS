...
def _percentile_of_score(a, score, axis, xp):
    ...
    """
    B = a.shape[axis]
    nonzeros = xp.count_nonzero(a < score, axis=axis) + xp.count_nonzero(
        a <= score, axis=axis
    )
...
def _bca_interval(data, statistic, axis, alpha, theta_hat_b, batch, xp):
    ...
    percentile = xp.asarray(
        percentile, dtype=theta_hat.dtype, device=xp_device(theta_hat)
    )
    ...
    theta_hat_ji = [xp.concat(theta_hat_i, axis=-1) for theta_hat_i in theta_hat_ji]
    ...
    theta_hat_j_dot = [
        xp.mean(theta_hat_i, axis=-1, keepdims=True) for theta_hat_i in theta_hat_ji
    ]
...
def _bootstrap_iv(
    ...
    if not vectorized:
        if not is_numpy(xp):
            message = (
                f"When using array library {xp.__name__}, `func` must be "
    ...
    if (
        not bootstrap_result or not xp_size(bootstrap_result.bootstrap_distribution)
...
def bootstrap(
    ...
    *,
    ...
    for k in range(0, n_resamples, batch_nominal):
        ...
        for sample in data:
            resample = _bootstrap_resample(
                sample, n_resamples=batch_actual, rng=rng, xp=xp
            )
    ...
    if method == "bca":
        ...
        alpha = xp.asarray(
            alpha, dtype=theta_hat_b.dtype, device=xp_device(theta_hat_b)
        )
...
def _power_iv(
    ...
    if (
        not xp.isdtype(significance.dtype, "real floating")
        or xp.min(significance) < 0
        or xp.max(significance) > 1
    ):
    ...
    if not vectorized:
        if not is_numpy(xp):
            message = (
                f"When using array library {xp.__name__}, `test` must be "
    ...
    *alternative* hypothesis and calculate the power.
    ...
    for i in range(nobs.shape[0]):
        ...
        for k in range(0, n_resamples, batch_nominal):
            batch_actual = min(batch_nominal, n_resamples - k)
            resamples = [
                rvs_j(size=(batch_actual, int(nobs_ij)), **kwargs_i)
...
def _calculate_null_both(data, statistic, n_permutations, batch, rng=None, *, xp):
    """
...
def _calculate_null_pairings(data, statistic, n_permutations, batch, rng=None, *, xp):
    """
...
def _calculate_null_samples(data, statistic, n_permutations, batch, rng=None, *, xp):
    """
    ...
    return _calculate_null_pairings(
        data, statistic_wrapped, n_permutations, batch, rng, xp=xp
    )
...
def _permutation_test_iv(
    ...
    n_resamples_int = int(n_resamples) if not math.isinf(n_resamples) else xp.inf
    if n_resamples != n_resamples_int or n_resamples_int <= 0:
        raise ValueError("`n_resamples` must be a positive integer.")
...
def permutation_test(
    ...
    *difference* between two paired elements. Accordingly, if ``data`` contains
    ...
    null_distribution, n_resamples, exact_test = calculate_null(
        *null_calculator_args, xp=xp
    )
