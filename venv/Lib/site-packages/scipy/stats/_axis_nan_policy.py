...
def _add_reduced_axes(res, reduced_axes, keepdims, xp=np):
    ...
    return (
        [
            (
                xpx.expand_dims(output, axis=reduced_axes)
    ...
    def axis_nan_policy_decorator(hypotest_fun_in):
        ...
        def axis_nan_policy_wrapper(*args, _no_deco=False, **kwds):
            ...
            if axis is None:
                ...
                samples = [
                    xp.moveaxis(sample, axis, tuple(range(-len(axis), 0)))
                    for sample in samples
                ]
                ...
                new_shapes = [
                    shape[:-n_axes] + (math.prod(shape[-n_axes:]),) for shape in shapes
                ]
                samples = [
                    xp.reshape(sample, new_shape)
            ...
            if empty_output is not None and (
                is_too_small(samples, kwds) or xp_size(empty_output) == 0
            ):
            ...
            if not is_numpy(xp) and "nan_policy" in kwds:
                msg = (
                    "Use of `nan_policy` is incompatible with multidimensional "
                    "non-NumPy arrays."
                )
                raise NotImplementedError(msg)
