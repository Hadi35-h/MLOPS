    ...
    **Interpolate and extrapolate a 2D dataset**
    ...
    def __call__(self, xi, method=None, *, nu=None):
        ...
        if method == "linear":
            ...
            if (
                ndim == 2
                and hasattr(self._values, "dtype")
                and self._values.ndim == 2
                and self._values.flags.writeable
                and self._values.dtype in (np.float64, np.complex128)
                and self._values.dtype.byteorder == "="
            ):
                ...
                result = evaluate_linear_2d(
                    self._values, indices, norm_distances, self._grid, out
                )
            else:
        ...
        if self.bounds_error:
            for i, p in enumerate(xi.T):
                if not np.logical_and(
                    np.all(self._grid[i][0] <= p), np.all(p <= self._grid[i][-1])
                ):
                    raise ValueError(
    ...
    def _evaluate_spline(self, xi, method):
        ...
        # last variable of xi.
        last_dim = n - 1
        first_values = _eval_func(self._grid[last_dim], values, xi[:, last_dim], k)
        ...
        for j in range(m):
            ...
            for i in range(last_dim - 1, -1, -1):
                ...
                folded_values = _eval_func(self._grid[i], folded_values, xi[j, i], k)
