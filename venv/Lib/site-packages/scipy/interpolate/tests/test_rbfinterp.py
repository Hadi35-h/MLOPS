...
class _TestRBFInterpolator:
    ...
    def test_extreme_domains(self, kernel, xp):
        ...
        if kernel in _SCALE_INVARIANT:
            ...
            yitp2 = self.build(x * scale + shift, y, kernel=kernel)(
                xitp * scale + shift
            )
        else:
            yitp1 = self.build(x, y, epsilon=5.0, kernel=kernel)(xitp)
            yitp2 = self.build(
                x * scale + shift, y, epsilon=5.0 / scale, kernel=kernel
            )(xitp * scale + shift)
    ...
    def test_vector_data(self, xp):
        ...
        y = xp.stack(
            [_2d_test_function(x, xp), _2d_test_function(xp.flip(x, axis=1), xp)]
        ).T
    ...
    def test_smoothing_misfit(self, kernel, xp):
        ...
        for smoothing in smoothing_range:
            ysmooth = self.build(x, y, epsilon=1.0, smoothing=smoothing, kernel=kernel)(
                x
            )
