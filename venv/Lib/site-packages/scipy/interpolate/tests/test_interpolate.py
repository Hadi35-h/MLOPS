...
class TestAkima1DInterpolator:
    def test_eval(self, xp):
        x = xp.arange(0.0, 11.0, dtype=xp.float64)
        y = xp.asarray(
            [0.0, 2.0, 1.0, 3.0, 2.0, 6.0, 5.5, 5.5, 2.7, 5.1, 3.0], dtype=xp.float64
        )
    ...
    def test_linear_interpolant_edge_case_2d(self, xp):
        ...
        y = xp.stack((x, 2.0 * x, 3.0 * x, 4.0 * x), axis=1)

        ak = Akima1DInterpolator(x, y)
        xi = xp.asarray([0.5, 1.0])
        yi = xp.asarray([[0.5, 1.0, 1.5, 2.0], [1.0, 2.0, 3.0, 4.0]], dtype=xp.float64)
        xp_assert_close(ak(xi), yi)
...
class TestPPoly:
    ...
    def test_periodic(self, xp):
        ...
        xp_assert_close(p(1.3), xp.asarray(1 * 0.3**2 + 2 * 0.3 + 3, dtype=xp.float64))
        xp_assert_close(
            p(-0.3),
            xp.asarray(4 * (0.7 - 0.5) ** 2 + 5 * (0.7 - 0.5) + 6, dtype=xp.float64),
        )
...
class TestBPoly:
    def test_simple(self, xp):
        ...
        c = xp.asarray([[3]])
        bp = BPoly(c, x)
        xp_assert_close(bp(0.1), xp.asarray(3.0, dtype=xp.float64))

    def test_simple2(self, xp):
        x = xp.asarray([0, 1])
        c = xp.asarray([[3], [1]])
        bp = BPoly(c, x)  # 3*(1-x) + 1*x
        xp_assert_close(bp(0.1), xp.asarray(3 * 0.9 + 1.0 * 0.1, dtype=xp.float64))
    ...
    def test_simple3(self, xp):
        ...
        xp_assert_close(
            bp(0.2),
            xp.asarray(
                3 * 0.8 * 0.8 + 1 * 2 * 0.2 * 0.8 + 4 * 0.2 * 0.2, dtype=xp.float64
            ),
        )
    ...
    def test_simple4(self, xp):
        ...
        xp_assert_close(
            bp(0.3),
            xp.asarray(
                0.7**3 + 3 * 0.7**2 * 0.3 + 3 * 0.7 * 0.3**2 + 2 * 0.3**3,
                dtype=xp.float64,
            ),
        )
...
class TestBPolyCalculus:
    def test_derivative(self, xp):
        ...
        xp_assert_close(
            xp.stack([bp(0.4, nu) for nu in [1, 2, 3]]),
            xp.asarray([-6 * (1 - 0.4), 6.0, 0.0], dtype=xp.float64),
        )
        xp_assert_close(
            xp.stack([bp(1.7, nu) for nu in [1, 2, 3]]),
            xp.asarray([0.7, 1.0, 0], dtype=xp.float64),
        )
    ...
    def test_antiderivative_simple(self, xp):
        ...
        xp_assert_close(
            bi(xx),
            xp.where(xx < 1, xx**2 / 2.0, 0.5 * xx * (xx / 2.0 - 1) + 3.0 / 4),
            atol=1e-12,
            rtol=1e-12,
        )
    ...
    def test_integrate_periodic(self, xp):
        ...
        xp_assert_close(
            P.integrate(3.5 + 12, 5 + 12), xp.asarray(I(2) - I(1) + I(4) - I(3.5))
        )
        xp_assert_close(
            P.integrate(3.5, 5 + 12),
