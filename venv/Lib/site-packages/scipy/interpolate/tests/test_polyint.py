...
def check_shape(
    interpolator_cls, x_shape, y_shape, deriv_shape=None, axis=0, extra_args=None
):
...
class TestKrogh:
    ...
    def test_int_inputs(self):
        ...
        xp_assert_close(
            abs((f(x) - offset_cdf) / f.derivative(x, 1)),
            np.zeros_like(offset_cdf),
            atol=1e-10,
        )
...
class TestBarycentric:
    ...
    def test_low_derivatives(self):
        ...
        D = P.derivatives(self.test_xs, len(self.xs) + 2)
        for i in range(D.shape[0]):
            xp_assert_close(self.true_poly.deriv(i)(self.test_xs), D[i], atol=1e-12)
    ...
    def test_high_derivative(self):
        ...
        for i in range(len(self.xs), 5 * len(self.xs)):
            xp_assert_close(P.derivative(self.test_xs, i), np.zeros(len(self.test_xs)))
...
class TestPCHIP:
    ...
    def test_nag(self):
        ...
        xp_assert_close(result[:, 1], pch(result[:, 0]), rtol=0.0, atol=5e-5)
    ...
    def test_pchip_interpolate(self):
        assert_array_almost_equal(
            pchip_interpolate([1, 2, 3], [4, 5, 6], [0.5], der=1), np.asarray([1.0])
        )

        assert_array_almost_equal(
            pchip_interpolate([1, 2, 3], [4, 5, 6], [0.5], der=0), np.asarray([3.5])
        )
...
class TestCubicSpline:
    ...
    def check_correctness(S, bc_start="not-a-knot", bc_end="not-a-knot", tol=1e-14):
        ...
        xp_assert_close(c[1, 1:], 3 * c[0, :-1] * dxi + c[1, :-1], rtol=tol, atol=tol)
        ...
        if bc_start == "not-a-knot":
            ...
            xp_assert_close(S(x[0], 1), np.zeros_like(S(x[0], 1)), rtol=tol, atol=tol)
        elif bc_start == "natural":
            xp_assert_close(S(x[0], 2), np.zeros_like(S(x[0], 2)), rtol=tol, atol=tol)
        else:
        ...
        if bc_end == "not-a-knot":
            ...
            xp_assert_close(S(x[-1], 1), np.zeros_like(S(x[-1], 1)), rtol=tol, atol=tol)
        elif bc_end == "natural":
            xp_assert_close(
                S(x[-1], 2), np.zeros_like(S(x[-1], 2)), rtol=2 * tol, atol=2 * tol
            )
        else:
            order, value = bc_end
