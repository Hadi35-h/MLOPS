...
def _build_and_solve_system(y, d, smoothing, kernel, epsilon, powers, xp):
    ...
    """
    lhs, rhs, shift, scale = _build_system(y, d, smoothing, kernel, epsilon, powers, xp)
    try:
...
def _build_system(y, d, smoothing, kernel, epsilon, powers, xp):
    ...
    lhs = xp.concat(
        ...
        axis=0,
    ) + xp.diag(xp.concat([smoothing, xp.zeros(r)]))
...
def _build_evaluation_coefficients(x, y, kernel, epsilon, powers, shift, scale, xp):
    ...
    vec = xp.concat(
        [
            kernel_func(
                xp.linalg.vector_norm(xeps[:, None, :] - yeps[None, :, :], axis=-1), xp
            ),
