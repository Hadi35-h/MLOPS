...
class _PPolyBase:
    ...
    def _get_dtype(self, dtype):
        if np.issubdtype(dtype, np.complexfloating) or np.issubdtype(
            self._c.dtype, np.complexfloating
        ):
    ...
    def extend(self, c, x):
        ...
        c2 = np.zeros(
            (k2, self._c.shape[1] + c.shape[1]) + self._c.shape[2:], dtype=dtype
        )
...
class PPoly(_PPolyBase):
    ...
    def antiderivative(self, nu=1):
        ...
        c = np.zeros(
            (self._c.shape[0] + nu, self._c.shape[1]) + self._c.shape[2:],
            dtype=self._c.dtype,
        )
        c[:-nu] = self._c
        ...
        self._ensure_c_contiguous()
        _ppoly.fix_continuity(c.reshape(c.shape[0], c.shape[1], -1), self._x, nu - 1)
