...
class Interpolation:
    ...
    def __init__(self, pb, options):
        ...
        close_xl_idx = (pb.bounds.xl + 0.5 * options[Options.RHOBEG] < self.x_base) & (
        ...
        close_xu_idx = (self.x_base < pb.bounds.xu - 0.5 * options[Options.RHOBEG]) & (
...
def build_system(interpolation):
    ...
    # difficulties.
    if _cache is not None and np.array_equal(interpolation.xpt, _cache["xpt"]):
...
class Models:
    ...
    def cub_hess_prod(self, v, mask=None):
        ...
        return np.reshape(
            [model.hess_prod(v, self.interpolation) for model in self._get_cub(mask)],
            (-1, self.n),
        )
    ...
    def ceq_hess_prod(self, v, mask=None):
        ...
        return np.reshape(
            [model.hess_prod(v, self.interpolation) for model in self._get_ceq(mask)],
            (-1, self.n),
        )
    ...
    def _check_interpolation_conditions(self):
        ...
        for k in range(self.npt):
            error_fun = np.max(
                [
                    error_fun,
                    np.abs(self.fun(self.interpolation.point(k)) - self.fun_val[k]),
                ]
            )
            error_cub = np.max(
                np.abs(self.cub(self.interpolation.point(k)) - self.cub_val[k, :]),
                initial=error_cub,
            )
            error_ceq = np.max(
                np.abs(self.ceq(self.interpolation.point(k)) - self.ceq_val[k, :]),
                initial=error_ceq,
            )
