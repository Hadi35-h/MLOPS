...
class TestDualAnnealing:
    ...
    def func(self, x, args=()):
        ...
        if args:
            ...
            shift = 0
        y = (
            np.sum((x - shift) ** 2 - 10 * np.cos(2 * np.pi * (x - shift)))
            + 10 * np.size(x)
            + shift
        )
    ...
    def test_visiting_dist_high_temperature(self, qv):
        ...
        assert_array_less(1e10, np.max(values))
    ...
    def test_low_dim_no_ls(self):
        ret = dual_annealing(
            self.func, self.ld_bounds, no_local_search=True, seed=self.seed
        )
        assert_allclose(ret.fun, 0.0, atol=1e-4)

    @pytest.mark.fail_slow(10)
    def test_high_dim_no_ls(self):
        ret = dual_annealing(
            self.func, self.hd_bounds, no_local_search=True, rng=self.seed
        )
        assert_allclose(ret.fun, 0.0, atol=1.2e-4)
    ...
    def test_nb_fun_call_no_ls(self):
        self.nb_fun_call.c = 0
        ret = dual_annealing(
            self.func, self.ld_bounds, no_local_search=True, rng=self.seed
        )
        assert_equal(self.nb_fun_call.c, ret.nfev)
    ...
    def test_deprecated_local_search_options_bounds(self):
        ...
        with pytest.warns(RuntimeWarning, match=r"Method CG cannot handle "):
            dual_annealing(
                func, bounds=bounds, minimizer_kwargs={"method": "CG", "bounds": bounds}
            )
    ...
    def test_minimizer_kwargs_bounds(self):
        ...
        # Test bounds can be passed (see gh-10831)
        dual_annealing(
            func, bounds=bounds, minimizer_kwargs={"method": "SLSQP", "bounds": bounds}
        )
    ...
    def test_max_fun_ls(self):
        ret = dual_annealing(self.func, self.ld_bounds, maxfun=100, rng=self.seed)
    ...
    def test_max_fun_no_ls(self):
        ret = dual_annealing(
            self.func, self.ld_bounds, no_local_search=True, maxfun=500, rng=self.seed
        )
        assert ret.nfev <= 500
        assert not ret.success

    def test_maxiter(self):
        ret = dual_annealing(self.func, self.ld_bounds, maxiter=700, rng=self.seed)
        assert ret.nit <= 700

    # Testing that args are passed correctly for dual_annealing
    def test_fun_args_ls(self):
        ret = dual_annealing(
            self.func, self.ld_bounds, args=((3.14159,)), rng=self.seed
        )
    ...
    def test_callback_stop(self):
        ...
        ret = dual_annealing(
            self.func, self.ld_bounds, callback=self.callback, rng=self.seed
        )
        assert ret.fun <= 1.0
    ...
    def test_wrong_restart_temp(self):
        assert_raises(
            ValueError, dual_annealing, self.func, self.ld_bounds, restart_temp_ratio=1
        )
        assert_raises(
            ValueError, dual_annealing, self.func, self.ld_bounds, restart_temp_ratio=0
        )
    ...
    def test_gradient_gnev(self):
        ...
        ret = dual_annealing(
            rosen, self.ld_bounds, minimizer_kwargs=minimizer_opts, rng=self.seed
        )
    ...
    @pytest.mark.parametrize(
        "new_e, temp_step, accepted, accept_rate",
        [
    ...
    def test_callable_jac_hess_with_args_gh11052(self):
        ...
        res2 = dual_annealing(
            ...
            bounds=[[0, 1], [0, 1]],
            rng=rng,
            minimizer_kwargs=dict(method="L-BFGS-B", jac=jac),
        )
        res3 = dual_annealing(
            f,
            args=(2,),
            bounds=[[0, 1], [0, 1]],
            rng=rng,
            minimizer_kwargs=dict(method="newton-cg", jac=jac, hess=hess),
        )
        res4 = dual_annealing(
            f,
            args=(2,),
            bounds=[[0, 1], [0, 1]],
            rng=rng,
            minimizer_kwargs=dict(method="newton-cg", jac=jac, hessp=hessp),
        )
