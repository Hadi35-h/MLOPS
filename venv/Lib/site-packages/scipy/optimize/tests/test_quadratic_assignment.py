...
class QAPCommonTests:
    ...
    def test_accuracy_1(self):
        ...
        res = quadratic_assignment(
            A, B, method=self.method, options={"rng": rng, "maximize": False}
        )
        ...
        res = quadratic_assignment(
            A, B, method=self.method, options={"rng": rng, "maximize": True}
        )
    ...
    def test_accuracy_3(self):
        ...
        # basic minimization
        res = quadratic_assignment(A, B, method=self.method, options={"rng": rng})
        ...
        res = quadratic_assignment(
            A, B, method=self.method, options={"partial_match": seed, "rng": rng}
        )
        assert_(11156 <= res.fun < 21000)
        ...
        empty = np.empty((0, 0))
        res = quadratic_assignment(
            empty, empty, method=self.method, options={"rng": rng}
        )
        assert_equal(res.nit, 0)
    ...
    def test_deprecation_future_warnings(self):
        ...
        with pytest.warns(FutureWarning, match="The NumPy global RNG was seeded*"):
            np.random.seed(0)
            quadratic_assignment(A, B, method=self.method, options={"maximize": False})

        with pytest.warns(FutureWarning, match="The behavior when the rng option*"):
            quadratic_assignment(
                A, B, method=self.method, options={"rng": 0, "maximize": False}
            )
...
class TestFAQ(QAPCommonTests):
    ...
    def test_specific_input_validation(self):
        ...
        with pytest.raises(
            ValueError, match="`P0` matrix must have shape m' x m', where m'=n-m"
        ):
            quadratic_assignment(
                np.identity(4), np.identity(4), options={"P0": np.ones((3, 3))}
            )
...
class Test2opt(QAPCommonTests):
    ...
    def test_partial_guess(self):
        ...
        res2 = quadratic_assignment(
            A, B, method=self.method, options={"rng": rng, "partial_guess": guess}
        )
        fix = [2, 4]
        ...
        res3 = quadratic_assignment(
            ...
            B,
            method=self.method,
            options={"rng": rng, "partial_guess": guess, "partial_match": match},
        )
    ...
    def test_specific_input_validation(self):
        ...
        _rm = _range_matrix
        with pytest.raises(
            ValueError, match="`partial_guess` can have only as many entries as"
        ):
            quadratic_assignment(
                np.identity(3),
                np.identity(3),
        ...
        with pytest.raises(
            ValueError, match="`partial_guess` entries must be less than number"
        ):
            quadratic_assignment(
                np.identity(5),
                np.identity(5),
        ...
        with pytest.raises(
            ValueError, match="`partial_guess` column entries must be unique"
        ):
            quadratic_assignment(
                np.identity(3),
                np.identity(3),
...
class TestQAPOnce:
    ...
    def test_common_input_validation(self):
        ...
        with pytest.raises(
            ValueError, match="`A` and `B` matrices must be of equal size"
        ):
            quadratic_assignment(
                rng.random((3, 3)),
                rng.random((4, 4)),
        ...
        _rm = _range_matrix
        with pytest.raises(
            ValueError, match="`partial_match` can have only as many seeds as"
        ):
            quadratic_assignment(
                np.identity(3), np.identity(3), options={"partial_match": _rm(5, 2)}
            )
        # test for only two seed columns
        ...
        # seeds can't have values greater than number of nodes
        with pytest.raises(
            ValueError, match="`partial_match` entries must be less than number"
        ):
            quadratic_assignment(
                np.identity(5),
                np.identity(5),
        ...
        # columns of seed matrix must be unique
        with pytest.raises(
            ValueError, match="`partial_match` column entries must be unique"
        ):
            quadratic_assignment(
                np.identity(3),
                np.identity(3),
