...
class TestODR:
    ...
    def test_explicit(self):
        explicit_mod = Model(
            ...
            meta=dict(name="Sample Explicit Model", ref="ODRPACK UG, pg. 39"),
        )
        explicit_dat = Data(
        ...
        assert_array_almost_equal(
            out.beta,
            np.array(
                [1.2646548050648876e03, -5.4018409956678255e01, -8.7849712165253724e-02]
            ),
        )
        assert_array_almost_equal(
            out.sd_beta,
        ...
        assert_array_almost_equal(
            ...
            np.array(
                [
                    [
                        4.4949592379003039e-01,
                        -3.7421976890364739e-01,
                        -8.0978217468468912e-04,
                    ],
                    [
                        -3.7421976890364739e-01,
                        1.0529686462751804e00,
    ...
    def implicit_fcn(self, B, x):
        return (
            B[2] * np.power(x[0] - B[0], 2)
            + 2.0 * B[3] * (x[0] - B[0]) * (x[1] - B[1])
            + B[4] * np.power(x[1] - B[1], 2)
            - 1.0
        )
    ...
    def test_implicit(self):
        ...
        implicit_odr = ODR(
            implicit_dat, implicit_mod, beta0=[-1.0, -3.0, 0.09, 0.02, 0.08]
        )
        ...
        assert_allclose(
            ...
            np.array(
                [
                    [
                        2.1089274602333052e00,
                        -1.9437686411979040e00,
                        7.0263550868344446e-02,
                        -4.7175267373474862e-02,
                        5.2515575927380355e-02,
                    ],
                    [
                        -1.9437686411979040e00,
                        2.0481509222414456e00,
                        -6.1600515853057307e-02,
                        4.6268827806232933e-02,
                        -5.8822307501391467e-02,
                    ],
                    [
                        7.0263550868344446e-02,
                        -6.1600515853057307e-02,
                        2.8659542561579308e-03,
                        -1.4628662260014491e-03,
    ...
    def multi_fcn(self, B, x):
        ...
        r = (B[0] - B[1]) * np.power(
            np.sqrt(np.power(1.0 + omega * ctheta, 2) + np.power(omega * stheta, 2)),
            -B[4],
        )
    ...
    def test_multi(self):
        multi_mod = Model(
            self.multi_fcn,
            meta=dict(name="Sample Multi-Response Model", ref="ODRPACK UG, pg. 56"),
        )
        ...
        multi_dat = Data(multi_x, multi_y, wd=1e-4 / np.power(multi_x, 2), we=multi_we)
        multi_odr = ODR(
            multi_dat,
            multi_mod,
            beta0=[4.0, 2.0, 7.0, 0.4, 0.5],
            delta0=multi_delta,
            ifixx=multi_ifixx,
        )
    ...
    def test_pearson(self):
        ...
        assert_array_almost_equal(
            ...
            np.array(
                [
                    [0.0854275622946333, -0.0161807025443155],
        ...
        assert_array_almost_equal(
            ...
            np.array(
                [
                    [0.6391799462548782, -0.1955657291119177],
    ...
    def lorentz(self, beta, x):
        return (
            ...
            * beta[2]
            / np.sqrt(
                np.power(x * x - beta[2] * beta[2], 2.0) + np.power(beta[1] * x, 2.0)
            )
        )
    ...
    def test_lorentz(self):
        ...
        assert_array_almost_equal(
            out.beta,
            np.array(
                [1.4306780846149925e03, 1.3390509034538309e-01, 3.7798193600109009e00]
            ),
        )
        assert_array_almost_equal(
            out.sd_beta,
            np.array(
                [7.3621186811330963e-01, 3.5068899941471650e-04, 2.4451209281408992e-04]
            ),
        )
        assert_array_almost_equal(
            out.cov_beta,
            np.array(
                [
                    [
                        2.4714409064597873e-01,
                        -6.9067261911110836e-05,
                        -3.1236953270424990e-05,
                    ],
                    [
                        -6.9067261911110836e-05,
                        5.6077531517333009e-08,
    ...
    def test_ticket_11800(self):
        ...
        beta_solution = np.array(
            [
                2.62920235756665876536e00,
                -1.26608484996299608838e02,
                1.29703572775403074502e02,
                -1.88560985401185465804e00,
                7.83834160771274923718e01,
                -7.64124076838087091801e01,
            ]
        )
    ...
    def test_work_ind(self):
        ...
        assert_array_almost_equal(
            out.sd_beta, out.work[sd_ind : sd_ind + len(out.sd_beta)]
        )

    @pytest.mark.skipif(
        True,
        reason="Fortran I/O prone to crashing so better "
    ...
    def test_work_array_del_init(self):
        ...
        for rd in [rd0, rd1, rd2, rd3, rd4, rd5]:
            odr_obj = ODR(
                rd, linear_model, beta0=[0.4, 0.4], delta0=np.full(n_data, -0.1)
            )
