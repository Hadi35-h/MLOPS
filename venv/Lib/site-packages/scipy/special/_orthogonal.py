...
def roots_jacobi(n, alpha, beta, mu=False):
    ...
    if (alpha + beta) <= 1000:
        ...
        mu0 = np.exp(
            (alpha + beta + 1) * np.log(2.0) + _ufuncs.betaln(alpha + 1, beta + 1)
        )
    a = alpha
    b = beta
    if a + b == 0.0:
...
def roots_genlaguerre(n, alpha, mu=False):
    ...
    def df(n, x):
        return (
            n * _ufuncs.eval_genlaguerre(n, alpha, x)
            - (n + alpha) * _ufuncs.eval_genlaguerre(n - 1, alpha, x)
        ) / x
...
def hermite(n, monic=False):
    ...
    kn = 2**n
    p = orthopoly1d(
        x, w, hn, kn, wfunc, (-inf, inf), monic, lambda x: _ufuncs.eval_hermite(n, x)
    )
    return p
...
def roots_gegenbauer(n, alpha, mu=False):
    ...
    if alpha <= 170:
        ...
        inv_alpha = 1.0 / alpha
        coeffs = np.array(
            [0.000207186, -0.00152206, -0.000640869, 0.00488281, 0.0078125, -0.125, 1.0]
        )
        mu0 = coeffs[0]
...
def chebyt(n, monic=False):
    ...
    if n == 0:
        return orthopoly1d(
            [], [], pi, 1.0, wfunc, (-1, 1), monic, lambda x: _ufuncs.eval_chebyt(n, x)
        )
    n1 = n
    ...
    kn = 2 ** (n - 1)
    p = orthopoly1d(
        x, w, hn, kn, wfunc, (-1, 1), monic, lambda x: _ufuncs.eval_chebyt(n, x)
    )
    return p
...
def roots_legendre(n, mu=False):
    ...
    def df(n, x):
        return (
            -n * x * _ufuncs.eval_legendre(n, x) + n * _ufuncs.eval_legendre(n - 1, x)
        ) / (1 - x**2)
