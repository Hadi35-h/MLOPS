...
def _skip_or_tweak_alternative_backends(xp, nfo, dtypes, int_only):
    ...
    if any("float" in dtype for dtype in dtypes) and (
        (
            is_torch(xp)
            and f_name in ("rel_entr", "xlogy", "polygamma", "zeta", "xlog1py")
        )
        or (
            is_jax(xp)
            and f_name
...
def test_support_alternative_backends(xp, func, nfo, base_dtype, shapes):
    ...
    if int_only is None:
        ...
        dtypes = tuple("intp" if needs_int else base_dtype for needs_int in int_only)
    ...
    args_xp = [
        xp.asarray(arg, dtype=dtype_xp) if not needs_python_int else arg
        for arg, dtype_xp, needs_python_int in zip(args_np, dtypes_xp, python_int_only)
    ]

    args_np = [
        np.asarray(arg, dtype=dtype_np) if not needs_python_int else arg
        for arg, dtype_np, needs_python_int in zip(args_np, dtypes_np, python_int_only)
    ]
    ...
    if is_torch(xp) and xpx.default_dtype(xp) == xp.float32 and "float64" not in dtypes:
...
def test_support_alternative_backends_mismatched_dtypes(xp, func, nfo):
    ...
    args_np = [
        np.asarray(arg, dtype=dtype_np) for arg, dtype_np in zip(args_np, dtypes_np)
    ]
...
def test_support_alternative_backends_hypothesis(xp, func, nfo, data):
    ...
    res = nfo.wrapper(*args_xp)  # Also wrapped by lazy_xp_function
    ref = nfo.func(*args_np)  # Unwrapped ufunc
    if is_torch(xp) and xpx.default_dtype(xp) == xp.float32 and dtype != "float64":
        # int64 promotes like float32 on torch with default dtype = float32
        # cast reference if needed
        ref = np.float32(ref)
...
@pytest.mark.parametrize(
    ...
    [(nfo.wrapper, nfo.n_args, nfo.int_only, nfo.is_ufunc) for nfo in _special_funcs],
)
def test_ufunc_kwargs(func, n_args, int_only, is_ufunc):
    ...
    # out=
    args = [
        np.asarray([0.1, 0.2]) if not needs_int else np.asarray([1, 2])
