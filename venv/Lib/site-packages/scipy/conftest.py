...
if SCIPY_ARRAY_API:
    ...
    try:
        ...
        xp_available_backends.append(
            pytest.param(torch, id="torch", marks=pytest.mark.array_api_backends)
        )
    ...
    try:
        ...
        xp_available_backends.append(
            pytest.param(cupy, id="cupy", marks=pytest.mark.array_api_backends)
        )
    ...
    if isinstance(SCIPY_ARRAY_API, str) and SCIPY_ARRAY_API.lower() not in (
        ...
        if SCIPY_ARRAY_API_ != {"all"}:
            ...
            xp_available_backends = [
                param for param in xp_available_backends if param.id in SCIPY_ARRAY_API_
            ]
...
if hypothesis_available:
    ...
    SCIPY_HYPOTHESIS_PROFILE = os.environ.get(
        "SCIPY_HYPOTHESIS_PROFILE", "deterministic"
    )
