...
def freqz(b, a=1, worN=512, whole=False, plot=None, fs=2 * pi, include_nyquist=False):
    ...
    if _is_int_type(worN):
        ...
        # include end point
        w = xp.linspace(
            0, lastpoint, N, endpoint=include_nyquist and not whole, dtype=real_dtype
        )
...
def buttord(wp, ws, gpass, gstop, analog=False, fs=None):
    ...
    if filter_type == 1:  # low
        ...
        discr = xp.sqrt((passb[1] - passb[0]) ** 2 + 4 * W0**2 * passb[0] * passb[1])
        ...
        WN = -W0 * (passb[1] - passb[0]) / 2.0 + xp.sqrt(
...
def besselap(N, norm="phase", *, xp=None, device=None):
    ...
    z = xp.asarray([], device=device, dtype=xp.float64)
    return (z, xp.asarray(p, device=device, dtype=xp.complex128), float(k))
...
def iircomb(w0, Q, ftype="notch", fs=2.0, *, pass_zero=False, xp=None, device=None):
    ...
    sgn = -1.0 if negative_coef else 1
    xpx.at(b, 0).set(bx)
