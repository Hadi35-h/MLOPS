...
class TestGetWindow:
    ...
    def test_symmetric_periodic(self, xp):
        ...
        # overwrite parameter `fftbins`:
        xp_assert_close(get_window("bartlett_periodic", 5, fftbins=False, xp=xp), w_per)
...
def test_windowfunc_basics(window, window_name, params, xp):
    ...
    with warnings.catch_warnings():
        warnings.filterwarnings("ignore", "This window is not suitable", UserWarning)
        ...
        xp_assert_equal(
            window(1, *params, sym=False, xp=xp), xp.asarray([1.0], dtype=xp.float64)
        )
...
@pytest.mark.parametrize(
    "window,winstr",
    [make_xp_pytest_param(_WIN_FUNCS[winstr][0], winstr) for winstr in _winstr],
)
@make_xp_test_case(get_window)
