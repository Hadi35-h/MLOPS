...
class TestLocalMaxima1d:
    ...
    @pytest.mark.parametrize(
        ...
        [
            np.array([1.0, 0, 2]),
            np.array([3.0, 3, 0, 4, 4]),
            np.array([5.0, 5, 5, 0, 6, 6, 6]),
        ],
    )
...
class TestRidgeLines:
    ...
    def test_single_pass(self):
        ...
        identified_lines = _identify_ridge_lines(
            test_matr, max_distances, max(gaps) + 1
        )
...
class TestPeakProminences:
    ...
    def test_wlen(self):
        ...
        for wlen, i in [(8, 0), (7, 0), (6, 0), (5, 1), (3.2, 1), (3, 2), (1.1, 2)]:
            ...
            for prom, val in zip(proms, [3.0 - i, 0 + i, 6 - i]):
                assert prom == val
...
class TestPeakWidths:
    ...
    def test_basic(self):
        ...
        for rel_height, width_true, lip_true, rip_true in [
            (0.0, 0.0, 3.0, 3.0),  # raises warning
            (0.25, 1.0, 2.5, 3.5),
            (0.5, 2.0, 2.0, 4.0),
            (0.75, 3.0, 1.5, 4.5),
            (1.0, 4.0, 1.0, 5.0),
    ...
    def test_non_contiguous(self):
        ...
        xp_assert_equal(result, np.asarray([[0.75], [75], [0.75], [1.5]]))
    ...
    def test_mismatching_prominence_data(self):
        ...
        for i, (prominences, left_bases, right_bases) in enumerate(
            ...
            prominence_data = (
                np.array(prominences, dtype=np.float64),
    ...
    def test_intersection_rules(self):
        ...
        xp_assert_close(
            peak_widths(x, peaks=[5], rel_height=0), [(0.0,), (3.0,), (5.0,), (5.0,)]
        )
        # width_height == x counts as intersection -> nearest 1 is chosen
        xp_assert_close(
            peak_widths(x, peaks=[5], rel_height=2 / 3),
            [(4.0,), (1.0,), (3.0,), (7.0,)],
        )
...
def test_unpack_condition_args():
    ...
    assert (3.0, 4.5) == _unpack_condition_args((3.0, 4.5), x, peaks)
...
class TestFindPeaks:
    ...
    def test_plateau_size(self):
        ...
        xp_assert_equal(
            props["left_edges"], peaks - (plateau_sizes - 1) // 2, check_dtype=False
        )
        xp_assert_equal(
            props["right_edges"], peaks + plateau_sizes // 2, check_dtype=False
        )

        # Test conditions
        xp_assert_equal(
            find_peaks(x, plateau_size=4)[0], [11, 18, 33, 100], check_dtype=False
        )
        xp_assert_equal(
            find_peaks(x, plateau_size=(None, 3.5))[0], [1, 3, 7], check_dtype=False
        )
        xp_assert_equal(
            find_peaks(x, plateau_size=(5, 50))[0], [18, 33], check_dtype=False
        )
    ...
    def test_height_condition(self):
        ...
        xp_assert_equal(
            props["peak_heights"], np.array([1 / 3, 2.5, 4.0]), check_dtype=False
        )
        xp_assert_equal(
            find_peaks(x, height=0.5)[0], np.array([3, 5]), check_dtype=False
        )
        xp_assert_equal(
            find_peaks(x, height=(None, 3))[0], np.array([1, 3]), check_dtype=False
        )
        xp_assert_equal(
            find_peaks(x, height=(2, 3))[0], np.array([3]), check_dtype=False
        )
    ...
    def test_threshold_condition(self):
        ...
        xp_assert_equal(find_peaks(x, threshold=2)[0], np.array([3]), check_dtype=False)
        xp_assert_equal(
            find_peaks(x, threshold=3.5)[0], np.array([], dtype=int), check_dtype=False
        )
        xp_assert_equal(
            find_peaks(x, threshold=(None, 5))[0], np.array([1, 3]), check_dtype=False
        )
        xp_assert_equal(
            find_peaks(x, threshold=(None, 4))[0], np.array([1]), check_dtype=False
        )
        xp_assert_equal(
            find_peaks(x, threshold=(2, 4))[0],
            np.array([], dtype=int),
            check_dtype=False,
        )
    ...
    def test_prominence_condition(self):
        ...
        xp_assert_equal(
            properties["left_bases"], np.zeros_like(properties["left_bases"])
        )
        xp_assert_equal(
            properties["right_bases"], peaks_true[keep] + 1, check_dtype=False
        )
    ...
    def test_width_condition(self):
        ...
        xp_assert_close(props["width_heights"], np.asarray([1.0]))
...
class TestFindPeaksCwt:
    ...
    def test_find_peaks_exact(self):
        ...
        found_locs = find_peaks_cwt(
            test_data, widths, gap_thresh=2, min_snr=0, min_length=None
        )
        xp_assert_equal(
            found_locs,
            act_locs,
    ...
    def test_find_peaks_withnoise(self):
        ...
        found_locs = find_peaks_cwt(
            test_data, widths, min_length=15, gap_thresh=1, min_snr=noise_amp / 5
        )
        ...
        np.testing.assert_array_less(
            diffs, max_diffs, "Maximum location differed" + f"by more than {max_diffs}"
        )
    ...
    def test_find_peaks_window_size(self):
        ...
        found_locs = find_peaks_cwt(
            test_data, widths, gap_thresh=2, min_snr=3, min_length=None, window_size=20
        )
        assert found_locs.size == act_locs.size
