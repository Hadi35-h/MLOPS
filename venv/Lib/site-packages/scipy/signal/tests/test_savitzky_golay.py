...
def test_polyder(xp):
    ...
    for p, m, expected in cases:
        ...
        check_polyder(pp.T if pp.ndim == 2 else pp, m, ee.T if ee.ndim == 2 else ee, xp)
...
def test_sg_coeffs_large(xp):
    ...
    xp_assert_close(coeffs0, xp.flip(coeffs0), atol=5e-4 if is_torch(xp) else 1.5e-6)

    coeffs1 = savgol_coeffs(31, 9, deriv=1, xp=xp)
    xp_assert_close(coeffs1, -xp.flip(coeffs1), atol=1.2e-2 if is_torch(xp) else 1.5e-6)
...
def test_sg_filter_2d(xp):
    x = xp.asarray([[1.0, 2.0, 1.0], [2.0, 4.0, 2.0]])
    expected = xp.asarray([[1.0, 4.0 / 3, 1.0], [2.0, 8.0 / 3, 2.0]])
    y = savgol_filter(x, 3, 1, mode="constant")
...
def test_sg_filter_interp_edges(xp):
    ...
    x = xp.stack([t, 3 * t**2, t**3 - t])
    dx = xp.stack([xp.ones_like(t), 6 * t, 3 * t**2 - 1.0])
    d2x = xp.stack([xp.zeros_like(t), xp.full_like(t, 6), 6 * t])
