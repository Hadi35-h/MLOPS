...
def test_get_blas_funcs():
    ...
    f1 = get_blas_funcs(
        "axpy",
        (np.empty((2, 2), dtype=np.float64), np.empty((2, 2), dtype=np.complex64)),
    )
    assert_equal(f1.typecode, "z")
...
class TestFBLAS2Simple:
    ...
    def test_ger(self, f, dtype):
        ...
        assert_array_almost_equal(
            f(1, [1, 2], [3, 4], a=[[1, 2], [3, 4]]), [[4, 6], [9, 12]]
        )
        if dtype in COMPLEX_DTYPES:
            assert_array_almost_equal(f(1, [1j, 2], [3, 4]), [[3j, 4j], [6, 8]])
            assert_array_almost_equal(
                f(2, [1j, 2j, 3j], [3j, 4j]), [[6, 8], [12, 16], [18, 24]]
            )
    ...
    def test_syr2(self, f, dtype):
        ...
        resxy_reverse = np.triu(
            x[::-1, np.newaxis] * y[::-1] + y[::-1, np.newaxis] * x[::-1]
        )
        ...
        assert_allclose(
            f(1.0, q, q, incx=2, offx=2, incy=2, offy=10, n=3), resxy[:3, :3], rtol=rtol
        )
        # negative increments imply reversed vectors in blas
        assert_allclose(
            f(1.0, q, q, incx=-2, offx=2, incy=-2, offy=10), resxy_reverse, rtol=rtol
        )
    ...
    def test_her2(self, f, dtype):
        ...
        assert_allclose(
            f(1.0, u, v, incx=3, offx=1, incy=3, offy=1, n=3), resxy[:3, :3], rtol=rtol
        )
        # negative increments imply reversed vectors in blas
        assert_allclose(
            f(1.0, u, v, incx=-3, offx=1, incy=-3, offy=1), resxy_reverse, rtol=rtol
        )
    ...
    def test_gbmv(self, dtype):
        ...
        A = toeplitz(
            append(rng.random(kl + 1), zeros(m - kl - 1)),
        ...
        y1 = func(
            m=m, n=n, ku=ku, kl=kl, alpha=alpha, a=Ab, x=y, y=x, beta=beta, trans=1
        )
    ...
    def test_tbmv(self, dtype):
        ...
        if dtype in COMPLEX_DTYPES:
            A[nonzero(A)] += 1j * rng.random((k + 1) * n - (k * (k + 1) // 2)).astype(
                dtype
            )
...
class TestBLAS3Syrk:
    ...
    def test_syrk_wrong_c(self, f, dtype):
        # FIXME narrow down to _fblas.error
        with pytest.raises(Exception):
            f(a=self.a, alpha=1.0, c=np.ones((5, 8)))
...
class TestBLAS3Syr2k:
    ...
    def test_syr2k_wrong_c(self, f, dtype):
        with pytest.raises(Exception):
            f(a=self.a, b=self.b, alpha=1.0, c=np.zeros((15, 8)))
        # if C is supplied, it must have compatible dimensions
...
class TestTRMM:
    ...
    def setup_method(self):
        ...
        self.a2 = np.array(
            [[1, 1, 2, 3], [0, 1, 4, 5], [0, 0, 1, 6], [0, 0, 0, 1]], order="f"
        )
    ...
    def test_side(self, dtype):
        ...
        res = trmm(1.0, self.a2.astype(dtype), self.b2.astype(dtype), side=1)
        k = self.b2.shape[1]
        assert_allclose(
            res, self.b2 @ self.a2[:k, :k], rtol=0.0, atol=100 * np.finfo(dtype).eps
        )
    ...
    def test_ab_lower(self, f, dtype):
        ...
        expected = np.array(
            [[3.0, 4.0, -1.0], [-1.0, -2.0, 0.0]]
        )  # now a is lower triangular
        assert_array_almost_equal(result, expected)
...
@pytest.mark.xfail(run=False, reason="gh-16930")
def test_gh_169309():
    x = np.repeat(10, 9)
