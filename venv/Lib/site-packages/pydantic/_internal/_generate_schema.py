        ...
        with (
            ...
            with self._config_wrapper_stack.push(config):
                ...
                for field_name, annotation in annotations.items():
                    ...
                    update_field_from_config(
                        self._config_wrapper, field_name, field_info
                    )
            ...
            if len(filtered_choices) >= 2:
                ...
                new_union["choices"] = [
                    (
                        ...
                        and choice[0]["type"] != "missing-sentinel"
                        else (
                            inner
    ...
    def _get_unsupported_field_info_attributes(
        ...
        for (
            ...
            if (
                ...
                and (
                    unused_metadata_name
                    not in ("validation_alias", "serialization_alias")
...
_VALIDATOR_F_MATCH: Mapping[
    ...
    (
        "before",
        "no-info",
    ): lambda f, schema: core_schema.no_info_before_validator_function(f, schema),
    (
        "after",
        "no-info",
    ): lambda f, schema: core_schema.no_info_after_validator_function(f, schema),
    ...
    (
        "before",
        "with-info",
    ): lambda f, schema: core_schema.with_info_before_validator_function(f, schema),
    (
        "after",
        "with-info",
    ): lambda f, schema: core_schema.with_info_after_validator_function(f, schema),
    ("plain", "with-info"): lambda f, _: core_schema.with_info_plain_validator_function(
        f
    ),
    (
        "wrap",
        "with-info",
    ): lambda f, schema: core_schema.with_info_wrap_validator_function(f, schema),
}
...
def apply_model_validators(
    ...
    for validator in validators:
        ...
        if mode == "outer" and validator.info.mode == "before":
            continue
        info_arg = inspect_validator(
            validator.func, mode=validator.info.mode, type="model"
        )
