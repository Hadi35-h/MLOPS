...
def _count_physical_cores_win32_powershell():
    cmd = (
        "-NoProfile -Command (Get-CimInstance -ClassName Win32_Processor).NumberOfCores"
    )
    cpu_info = subprocess.run(
...
def _count_physical_cores_win32_ctypes():
    ...
    get_logical_processor_information = kernel32.GetLogicalProcessorInformationEx
    ...
    while offset < returned_length.value:
        ...
        processor_core_info = SYSTEM_LOGICAL_PROCESSOR_INFORMATION_EX.from_buffer(
            buf, offset
        )
