...
def check_hyphen_ok(label: str) -> bool:
    ...
    if label[2:4] == "--":
        raise IDNAError(
            "Label has disallowed hyphens in 3rd and 4th position", code="hyphen_3_4"
        )
    if label.startswith("-") or label.endswith("-"):
        raise IDNAError(
            "Label must not start or end with a hyphen", code="hyphen_start_end"
        )
    return True
...
def valid_contextj(label: str, pos: int) -> bool:
    ...
    if cp_value == 0x200D:
        return (
            pos > 0 and _combining_class(ord(label[pos - 1])) == _virama_combining_class
        )
...
def ulabel(label: str | bytes | bytearray) -> str:
    ...
    if _punycode(label) != label_bytes:
        raise IDNAError(
            "A-label is not the canonical Punycode encoding of its U-label",
