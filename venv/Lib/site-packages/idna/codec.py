...
class Codec(codecs.Codec):
    ...
    ) -> tuple[bytes, int]:  # ty: ignore[invalid-method-override]
        if errors != "strict":
            raise IDNAError(
                f'Unsupported error handling "{errors}"', code="unsupported_errors"
            )
    ...
    ) -> tuple[str, int]:  # ty: ignore[invalid-method-override]
        if errors != "strict":
            raise IDNAError(
                f'Unsupported error handling "{errors}"', code="unsupported_errors"
            )
    ...
    def _buffer_decode(
        ...
        if not isinstance(data, str):
            try:
                ...
                raise IDNAError(
                    "Invalid ASCII in A-label", code="invalid_ascii"
                ) from err
