...
class Config:
    ...
    def load(self) -> None:
        ...
        if self.ssl_context_factory is not None:
            ...
            if not isinstance(context, ssl.SSLContext):
                raise TypeError(
                    f"`ssl_context_factory` must return an `ssl.SSLContext`, got {type(context).__name__}"
                )
        ...
        if isinstance(self.http, str):
            http_protocol_class = import_from_string(
                HTTP_PROTOCOLS.get(self.http, self.http)
            )
    ...
    def get_loop_factory(self) -> Callable[[], asyncio.AbstractEventLoop] | None:
        if self.loop in LOOP_FACTORIES:
            loop_factory: Callable[..., Any] | None = import_from_string(
                LOOP_FACTORIES[self.loop]
            )
        else:
            try:
    ...
    def bind_socket(self) -> socket.socket:
        ...
        if self.uds is not None:  # pragma: py-win32
            ...
            color_message = (
                "Uvicorn running on "
                + style(sock_name_format, bold=True)
            ...
            color_message = (
                "Uvicorn running on "
                + style(fd_name_format, bold=True)
                + " (Press CTRL+C to quit)"
            )
