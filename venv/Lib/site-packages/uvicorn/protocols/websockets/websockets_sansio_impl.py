...
class WebSocketsSansIOProtocol(asyncio.Protocol):
        ...
        if self.curr_msg_data_type == "text":
            try:
                self.queue.put_nowait(
                    {"type": "websocket.receive", "text": data.decode()}
                )
    ...
    def handle_pong(self, event: Frame) -> None:
        ...
        if (
            ...
            or bytes(event.data) != self.pending_ping_payload
        ):
    ...
    async def send(self, message: ASGISendEvent) -> None:
        ...
        if not self.handshake_complete and self.initial_response is None:
            if message["type"] == "websocket.accept":
                ...
                and self.initial_response is None
            ):
                if not (100 <= message["status"] < 600):
                    raise RuntimeError(
                        "Invalid HTTP status code '%d' in response." % message["status"]
                    )
                self.logger.info(
                    '%s - "WebSocket %s" %d',
                    get_client_addr(self.scope),
            ...
            if message["type"] == "websocket.http.response.body":
                ...
                if not message.get("more_body", False):
                    ...
                    response = Response(
                        status_code, STATUS_PHRASES[status_code], response_headers, body
                    )
                    self.queue.put_nowait(
