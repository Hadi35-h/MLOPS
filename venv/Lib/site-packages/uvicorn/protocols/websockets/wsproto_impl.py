...
class WSProtocol(asyncio.Protocol):
    ...
    def handle_message(self, event: events.TextMessage | events.BytesMessage) -> None:
        try:
            ...
            reason = (
                f"Message exceeds the maximum size ({self.config.ws_max_size} bytes)"
            )
            self.queue.put_nowait(
                {"type": "websocket.disconnect", "code": 1009, "reason": reason}
            )
            if not self.transport.is_closing():
                self.transport.write(
                    self.conn.send(
                        wsproto.events.CloseConnection(code=1009, reason=reason)
                    )
                )
                self.transport.close()
            return
    ...
    def handle_pong(self, event: events.Pong) -> None:
        ...
        if (
            self.pending_ping_payload is None
            or bytes(event.payload) != self.pending_ping_payload
        ):
    ...
    def send_keepalive_ping(self) -> None:
        ...
        self.transport.write(
            self.conn.send(wsproto.events.Ping(payload=self.pending_ping_payload))
        )
        if self.ping_timeout is not None:
            self.pong_timer = self.loop.call_later(
                self.ping_timeout, self.keepalive_timeout
            )
    ...
    def keepalive_timeout(self) -> None:
        ...
        if self.logger.level <= TRACE_LOG_LEVEL:
            prefix = "%s:%d - " % self.client if self.client else ""
            self.logger.log(
                TRACE_LOG_LEVEL, "%sWebSocket keepalive ping timeout", prefix
            )
        reason = "keepalive ping timeout"
        self.transport.write(
            self.conn.send(wsproto.events.CloseConnection(code=1011, reason=reason))
        )
        self.close_sent = True
        self.transport.close()
    ...
    async def send(self, message: ASGISendEvent) -> None:
        ...
        if not self.handshake_complete:
            ...
            try:
                if message["type"] == "websocket.send":
                    ...
                    if not self.transport.is_closing():
                        ...
                        self.close_timer = self.loop.call_later(
                            self.close_timeout, self.transport.close
                        )
            ...
            if message["type"] == "websocket.http.response.body":
                ...
                raise RuntimeError(
                    f"Expected ASGI message 'websocket.http.response.body' but got '{message['type']}'."
                )
