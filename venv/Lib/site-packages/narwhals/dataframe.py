...
class BaseFrame(Generic[_FrameT]):
    ...
    def _flatten_and_extract(
        ...
        parse = partial(
            _parse_into_expr,
            backend=self._compliant._implementation,
            allow_literal=False,
        )
        all_exprs = chain(
    ...
    def filter(
        ...
        predicate = all_horizontal(
            *chain(
                flat_predicates, (col(name) == v for name, v in constraints.items())
            ),
            ignore_nulls=False,
        )
...
class DataFrame(BaseFrame[DataFrameT]):
    ...
    def filter(
        ...
        **constraints: Any,
        ...
        parsed_predicates = (
            (
                self._series.from_iterable("", p, backend=impl)
                if is_list_of(p, bool)
                else p
            )
            for p in predicates
        )
