...
class SQLExpr(
    ...
    def _from_elementwise_horizontal_op(
        cls, func: Callable[[Sequence[NativeExprT]], NativeExprT], *exprs: Self
    ) -> Self:
    ...
    def clip_lower(self, lower_bound: Self) -> Self:
        ...
        return self._with_elementwise(
            _clip, expression_args={"lower_bound": lower_bound}
        )
    ...
    def clip_upper(self, upper_bound: Self) -> Self:
        ...
        return self._with_elementwise(
            _clip, expression_args={"upper_bound": upper_bound}
        )
    ...
    def first(self, order_by: Sequence[str] = ()) -> Self:
        ...
        def window_f(
            ...
            return [
                self._window_expression(
                    self._first(expr, *(inputs.order_by or order_by)),
                    inputs.partition_by,
                )
                for expr in self(df)
            ]
    ...
    def last(self, order_by: Sequence[str] = ()) -> Self:
        ...
        def window_f(
            ...
            return [
                self._window_expression(
                    self._last(expr, *(inputs.order_by or order_by)),
                    inputs.partition_by,
                )
                for expr in self(df)
            ]
