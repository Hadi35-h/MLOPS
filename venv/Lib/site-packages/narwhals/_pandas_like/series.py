...
class PandasLikeSeries(EagerSeries[Any]):
    ...
    def _align_full_broadcast(cls, *series: Self) -> Sequence[Self]:
        ...
        target_length = max(
            length
            for length, s in zip(lengths, series, strict=False)
            if not s._broadcast
        )
    ...
    def _apply_pyarrow_compute_func(
        ...
        pc_func: Callable[[ChunkedArrayAny], ChunkedArrayAny],
    ) -> NativeSeriesT:
    ...
    def _apply_array_func(
        self,
        native: NativeSeriesT,
        array_func: Callable[[NativeSeriesT], NativeSeriesT],
    ) -> NativeSeriesT:
        native_cls = type(native)
